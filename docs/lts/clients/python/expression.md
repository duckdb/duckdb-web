...
The `Expression` class represents an instance of an [expression]({% link docs/lts/sql/expressions/overview.md %}).
...
| `.isin(*exprs: Expression)`    | Creates an [`IN` expression]({% link docs/lts/sql/expressions/in.md %}#in) against the provided expressions as the list         |
| `.isnotin(*exprs: Expression)` | Creates a [`NOT IN` expression]({% link docs/lts/sql/expressions/in.md %}#not-in) against the provided expressions as the list  |
