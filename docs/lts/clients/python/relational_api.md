...
- **params** : object
	...
	Optional query parameters. **Discouraged** due to [significant performance overhead]({% link docs/lts/clients/python/known_issues.md %}#parameterized-queries-in-relational-api). Use [`execute()`]({% link docs/lts/clients/python/dbapi.md %}#prepared-statements) for parameterized queries instead.
