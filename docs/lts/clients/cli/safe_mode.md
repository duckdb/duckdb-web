...
* The following [dot commands]({% link docs/lts/clients/cli/dot_commands.md %}) are disabled:
    * `.cd`
    * `.excel`
    * `.import`
    * `.log`
...
* The [`getenv` function]({% link docs/lts/clients/cli/overview.md %}#reading-environment-variables) is disabled.
* The [`enable_external_access` option]({% link docs/lts/configuration/overview.md %}#configuration-reference) is set to `false`. This implies that:
...
For more information on running DuckDB in secure environments, see the [“Securing DuckDB” page]({% link docs/lts/operations_manual/securing_duckdb/overview.md %}).
