...
settings can be changed later on using [`PRAGMA` statements]({% link docs/lts/configuration/pragmas.md %}) as well.
...
> Warning Do *not* use prepared statements to insert large amounts of data into DuckDB. See the [data import documentation]({% link docs/lts/data/overview.md %}) for better options.
...
The [Appender]({% link docs/lts/data/appender.md %}) is available in the DuckDB JDBC driver via the `org.duckdb.DuckDBAppender` class.
