...
Please see the [installation page]({% link install/index.html %}?environment=r) and [CRAN](https://cran.r-project.org/web/packages/duckdb/index.html) for details.
...
Also refer to the [data import documentation]({% link docs/lts/data/overview.md %}) for more options of efficiently importing data.
...
You can use the [`memory_limit` configuration option]({% link docs/lts/configuration/pragmas.md %}) to limit the memory use of DuckDB, e.g.:
