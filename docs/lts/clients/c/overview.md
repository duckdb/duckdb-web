...
DuckDB implements a custom C API modeled somewhat following the SQLite C API. The API is contained in the `duckdb.h` header. Continue to [Startup & Shutdown]({% link docs/lts/clients/c/connect.md %}) to get started, or check out the [Full API overview]({% link docs/lts/clients/c/api.md %}).
