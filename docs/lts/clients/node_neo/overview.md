...
It depends on low-level bindings that adhere closely to [DuckDB's C API]({% link docs/lts/clients/c/overview.md %}),
...
* Lossless & efficient support for values of all [DuckDB data types]({% link docs/lts/sql/data_types/overview.md %}).
* Wraps [released DuckDB binaries](https://github.com/duckdb/duckdb/releases) instead of rebuilding DuckDB.
* Built on [DuckDB's C API]({% link docs/lts/clients/c/overview.md %}); exposes more functionality.
...
Set [configuration options]({% link docs/lts/configuration/overview.md %}):
```ts
const instance = await DuckDBInstance.create('my_duckdb.db', {
  threads: '4'
});
