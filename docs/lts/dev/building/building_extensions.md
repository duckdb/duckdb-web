...
[Extensions]({% link docs/lts/extensions/overview.md %}) can be built from source and installed from the resulting local binary.
...
This option also accepts out-of-tree extensions such as [`delta`]({% link docs/lts/core_extensions/delta.md %}):
...
When this flag is set, the [`jemalloc` extension]({% link docs/lts/core_extensions/jemalloc.md %}) is built.
