...
If you are working in a memory-constrained environment, consider [limiting the number of threads]({% link docs/lts/configuration/pragmas.md %}#threads), e.g., by issuing:
...
Read more in the [“How to Tune Workloads” page]({% link docs/lts/guides/performance/how_to_tune_workloads.md %}#persistent-vs-in-memory-tables).
...
DuckDB can be built with both [glibc](https://www.gnu.org/software/libc/) (default) and [musl libc](https://www.musl-libc.org/) (see the [build guide]({% link docs/lts/dev/building/linux.md %})).
...
If you have a many-core CPU running on a system where DuckDB ships with [`jemalloc`]({% link docs/lts/core_extensions/jemalloc.md %}) as the default memory allocator, consider [enabling the allocator's background threads]({% link docs/lts/core_extensions/jemalloc.md %}#background-threads).
