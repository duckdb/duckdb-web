...
This guide requires the [`httpfs` extension]({% link docs/lts/core_extensions/httpfs/overview.md %}), which can be installed using the `INSTALL httpfs` SQL command. This only needs to be run once.

## Attaching to a Database over HTTPS

To connect to a DuckDB database via HTTPS, use the [`ATTACH` statement]({% link docs/lts/sql/statements/attach.md %}) as follows:
...
To connect to a DuckDB database via the S3 API, [configure the authentication]({% link docs/lts/guides/network_cloud_storage/s3_import.md %}#credentials-and-configuration) for your bucket (if required).
...
> Connecting to S3-compatible APIs such as the [Google Cloud Storage (`gs://`)]({% link docs/lts/guides/network_cloud_storage/gcs_import.md %}#attaching-to-a-database) is also supported.
