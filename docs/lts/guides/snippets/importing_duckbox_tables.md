...
By default, the DuckDB [CLI client]({% link docs/lts/clients/cli/overview.md %}) renders query results in the [duckbox format]({% link docs/lts/clients/cli/output_formats.md %}),
...
we can turn the table into a `│`-separated file and read it with DuckDB's [CSV reader]({% link docs/lts/data/csv/overview.md %}).
...
We can also create a [table macro]({% link docs/lts/sql/statements/create_macro.md %}#table-macros):
...
> Please consult the [Securing DuckDB page]({% link docs/lts/operations_manual/securing_duckdb/overview.md %}) for more details.
...
* Only the data types [supported by the CSV sniffer]({% link docs/lts/data/csv/auto_detection.md %}#type-detection) are parsed correctly. Values containing nested data will be parsed as a `VARCHAR`.
