...
DuckDB has a powerful [macro mechanism]({% link docs/lts/sql/statements/create_macro.md %}) that allows creating shorthands for common tasks.
...
Make sure that the [`httpfs` extension]({% link docs/lts/core_extensions/httpfs/overview.md %}) is installed:
...
To use it, make sure that the [`httpfs` extension]({% link docs/lts/core_extensions/httpfs/overview.md %}) is installed:
