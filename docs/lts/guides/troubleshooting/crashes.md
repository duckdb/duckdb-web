...
* **Internal errors:** an operation may result in an [`Internal Error`]({% link docs/lts/dev/internal_errors.md %}), e.g.:
...
* **Out of memory errors:** A DuckDB crash can also be a symptom of the operating system killing the process.
  ...
  If your DuckDB session is killed by the OOM reaper, consult the [“OOM errors” page]({% link docs/lts/guides/troubleshooting/oom_errors.md %})
...
DuckDB will then replay the write-ahead log and perform a [checkpoint operation]({% link docs/lts/sql/statements/checkpoint.md %}), restoring the database to the state before the crash.
...
you can try [building it from source]({% link docs/lts/dev/building/overview.md %}).
...
To pinpoint the specific rules that caused the crash, you can try to [selectively disable optimizer rules]({% link docs/lts/configuration/pragmas.md %}#selectively-disabling-optimizers). This way, your query can still benefit from the rest of the optimizer rules.
...
To understand whether this is the case, try reproducing the issue using plain SQL queries with the [DuckDB CLI client]({% link docs/lts/clients/cli/overview.md %}).
