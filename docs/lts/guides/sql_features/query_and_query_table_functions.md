...
The [`query_table`]({% link docs/lts/sql/functions/utility.md %}#query_tabletbl_name)
and [`query`]({% link docs/lts/sql/functions/utility.md %}#queryquery_string_literal)
...
When combined with the [`COLUMNS` expression]({% link docs/lts/sql/expressions/star.md %}#columns), we can write very generic SQL-only macros. For example, below is a custom version of `SUMMARIZE` that computes the `min` and `max` of every column in a table:
