...
To create a new table using the result from a query, use the [`CREATE TABLE ... AS SELECT` statement]({% link docs/lts/sql/statements/create_table.md %}#create-table--as-select-ctas):
...
For additional options, see the [Parquet loading reference]({% link docs/lts/data/parquet/overview.md %}).
