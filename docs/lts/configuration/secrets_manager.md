...
| ------------- | -------------------- | --------------------------------------------------------------------------------- |
| `azure`       | Azure Blob Storage   | [`azure`]({% link docs/lts/core_extensions/azure.md %})                       |
| `ducklake`    | DuckLake             | [`ducklake`](https://ducklake.select/docs/stable/duckdb/usage/connecting#secrets) |
| `gcs`         | Google Cloud Storage | [`httpfs`]({% link docs/lts/core_extensions/httpfs/s3api.md %})               |
| `http`        | HTTP and HTTPS       | [`httpfs`]({% link docs/lts/core_extensions/httpfs/https.md %})               |
| `huggingface` | Hugging Face         | [`httpfs`]({% link docs/lts/core_extensions/httpfs/hugging_face.md %})        |
| `iceberg`     | Iceberg REST Catalog | [`httpfs`]({% link docs/lts/core_extensions/httpfs/s3api.md %}), [`iceberg`]({% link docs/lts/core_extensions/iceberg/iceberg_rest_catalogs.md %}) |
| `mysql`       | MySQL                | [`mysql`]({% link docs/lts/core_extensions/mysql.md %})                       |
| `postgres`    | PostgreSQL           | [`postgres`]({% link docs/lts/core_extensions/postgres.md %})                 |
| `r2`          | Cloudflare R2        | [`httpfs`]({% link docs/lts/core_extensions/httpfs/s3api.md %})               |
| `s3`          | AWS S3               | [`httpfs`]({% link docs/lts/core_extensions/httpfs/s3api.md %})               |
...
Secrets can be created using the [`CREATE SECRET` SQL statement]({% link docs/lts/sql/statements/create_secret.md %}).
...
Secrets can be deleted using the [`DROP SECRET` statement]({% link docs/lts/sql/statements/create_secret.md %}#syntax-for-drop-secret), e.g.:
...
Secrets can be listed using the built-in table-producing function, e.g., by using the [`duckdb_secrets()` table function]({% link docs/lts/sql/meta/duckdb_table_functions.md %}#duckdb_secrets):
