...
Query the [metadata of a Parquet file]({% link docs/lts/data/parquet/metadata.md %}#parquet-metadata):
...
Query the [file metadata of a Parquet file]({% link docs/lts/data/parquet/metadata.md %}#parquet-file-metadata):
...
Query the [key-value metadata of a Parquet file]({% link docs/lts/data/parquet/metadata.md %}#parquet-key-value-metadata):
...
Query the [schema of a Parquet file]({% link docs/lts/data/parquet/metadata.md %}#parquet-schema):
...
> Parquet datasets differ based on the number of files, the size of individual files, the compression algorithm used, row group size, etc. These have a significant effect on performance. Please consult the [Performance Guide]({% link docs/lts/guides/performance/file_formats.md %}) for details.
...
There are a number of options exposed that can be passed to the `read_parquet` function or the [`COPY` statement]({% link docs/lts/sql/statements/copy.md %}).
...
| `encryption_config` | Configuration for [Parquet encryption]({% link docs/lts/data/parquet/encryption.md %}). | `STRUCT` | - |
...
| `union_by_name` | Whether the columns of multiple schemas should be [unified by name]({% link docs/lts/data/multiple_files/combining_schemas.md %}), rather than by position. | `BOOL` | `false` |
...
DuckDB also has support for writing to Parquet files using the `COPY` statement syntax. See the [`COPY` Statement page]({% link docs/lts/sql/statements/copy.md %}) for details, including all possible parameters for the `COPY` statement.
...
Write to Parquet file with [key-value metadata]({% link docs/lts/data/parquet/metadata.md %}):
...
DuckDB's `EXPORT` command can be used to export an entire database to a series of Parquet files. See the [“`EXPORT` statement” page]({% link docs/lts/sql/statements/export.md %}) for more details:
...
DuckDB supports reading and writing [encrypted Parquet files]({% link docs/lts/data/parquet/encryption.md %}).
