...
Alternatively, create a table without specifying the schema manually using a [`CREATE TABLE ... AS SELECT` statement]({% link docs/lts/sql/statements/create_table.md %}#create-table--as-select-ctas):
...
We can use the [`FROM`-first syntax]({% link docs/lts/sql/query_syntax/from.md %}#from-first-syntax) to omit `SELECT *`.
...
| `auto_detect` | [Auto detect CSV parameters]({% link docs/lts/data/csv/auto_detection.md %}). | `BOOL` | `true` |
...
| `dateformat` | [Date format]({% link docs/lts/sql/functions/dateformat.md %}) used when parsing and writing dates. | `VARCHAR` | (empty) |
...
| `hive_partitioning` | Interpret the path as a [Hive partitioned path]({% link docs/lts/data/partitioning/hive_partitioning.md %}). | `BOOL` | (auto-detected) |
...
| `names` or `column_names` | Column names, as a list. See [example]({% link docs/lts/data/csv/tips.md %}#provide-names-if-the-file-does-not-contain-a-header). | `VARCHAR[]` | (empty) |
...
| `rejects_scan` | Name of the [temporary table where information on faulty scans is stored]({% link docs/lts/data/csv/reading_faulty_csv_files.md %}#reject-scans). | `VARCHAR` | `reject_scans` |
| `rejects_table` | Name of the [temporary table where information on faulty lines is stored]({% link docs/lts/data/csv/reading_faulty_csv_files.md %}#reject-errors). | `VARCHAR` | `reject_errors` |
...
| `sample_size` | Number of sample lines for [auto detection of parameters]({% link docs/lts/data/csv/auto_detection.md %}). | `BIGINT` | 20480 |
...
| `timestampformat` | [Timestamp format]({% link docs/lts/sql/functions/dateformat.md %}) used when parsing and writing timestamps. | `VARCHAR` | (empty) |
| `timestamp_format` | Alias for `timestampformat`; only available in the `COPY` statement. | `VARCHAR` | (empty) |
| `types` or `dtypes` or `column_types` | Column types, as either a list (by position) or a struct (by name). See [example]({% link docs/lts/data/csv/tips.md %}#override-the-types-of-specific-columns). | `VARCHAR[]` or `STRUCT` | (empty) |
| `union_by_name` | Align columns from different files [by column name]({% link docs/lts/data/multiple_files/combining_schemas.md %}#union-by-name) instead of position. Using this option increases memory consumption. | `BOOL` | `false` |

> Tip DuckDB's CSV reader supports `UTF-8` (default), `UTF-16` and `Latin-1` encodings.
> For other encodings, you can either use [the `encodings` extension]({% link docs/lts/core_extensions/encodings.md %})
...
The `auto_type_candidates` option lets you specify the data types that should be considered by the CSV reader for [column data type detection]({% link docs/lts/data/csv/auto_detection.md %}#type-detection).
Usage example:
...
DuckDB supports reading erroneous CSV files. For details, see the [Reading Faulty CSV Files page]({% link docs/lts/data/csv/reading_faulty_csv_files.md %}).

## Order Preservation

The CSV reader respects the `preserve_insertion_order` [configuration option]({% link docs/lts/configuration/overview.md %}) to [preserve insertion order]({% link docs/lts/sql/dialect/order_preservation.md %}).
...
DuckDB can write CSV files using the [`COPY ... TO` statement]({% link docs/lts/sql/statements/copy.md %}#copy--to).
