...
| `hive_partitioning` | Whether or not to interpret the path as a [Hive partitioned path]({% link docs/lts/data/partitioning/hive_partitioning.md %}). | `BOOL` | (auto-detected) |
...
| `dateformat` | Specifies the date format to use when parsing dates. See [Date Format]({% link docs/lts/sql/functions/dateformat.md %}) | `VARCHAR` | `iso` |
...
| `timestampformat` | Specifies the date format to use when parsing timestamps. See [Date Format]({% link docs/lts/sql/functions/dateformat.md %}) | `VARCHAR` | `iso`|
| `union_by_name` | Whether the schema's of multiple JSON files should be [unified]({% link docs/lts/data/multiple_files/combining_schemas.md %}) | `BOOL` | `false` |
...
When the `json` extension is installed, `FORMAT json` is supported for `COPY FROM`, `IMPORT DATABASE`, as well as `COPY TO` and `EXPORT DATABASE`. See the [`COPY` statement]({% link docs/lts/sql/statements/copy.md %}) and the [`IMPORT` / `EXPORT` clauses]({% link docs/lts/sql/statements/export.md %}).
...
| `hive_partitioning` | Whether or not to interpret the path as a [Hive partitioned path]({% link docs/lts/data/partitioning/hive_partitioning.md %}). | `BOOL` | `false` |
| `ignore_errors` | Whether to ignore parse errors (only possible when `format` is `newline_delimited`) | `BOOL` | `false` |
| `maximum_depth` | Maximum nesting depth to which the automatic schema detection detects types. Set to -1 to fully detect nested JSON types | `BIGINT` | `-1` |
| `maximum_object_size` | The maximum size of a JSON object (in bytes) | `UINTEGER` | `16777216` |
