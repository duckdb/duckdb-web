...
Sequences can be used as `DEFAULT` values in [`CREATE TABLE` statements]({% link docs/lts/sql/statements/create_table.md %}). 

The example below uses a sequence to create an integer [primary key]({% link docs/lts/sql/constraints.md %}#primary-key-and-unique-constraint):
...
Sequences can also be added using the [`ALTER TABLE` statement]({% link docs/lts/sql/statements/alter_table.md %}). The following example adds an `id` column and fills it with values generated by the sequence:
