...
Attach a SQLite database for reading and writing (see the [`sqlite` extension]({% link docs/lts/core_extensions/sqlite.md %}) for more information):
...
For more details, see the [“Storage” page]({% link docs/lts/internals/storage.md %}#explicit-storage-versions).
...
To encrypt data, DuckDB can use either the built-in `mbedtls` library or the OpenSSL library from the [`httpfs` extension]({% link docs/lts/core_extensions/httpfs/overview.md %}). Note that the OpenSSL versions are much faster due to hardware acceleration, so make sure to load the `httpfs` for good encryption performance:
...
> Warning Closing the connection, e.g., invoking the [`close()` function in Python]({% link docs/lts/clients/python/dbapi.md %}#connection), does not release the locks held on the database files as the file handles are held by the main DuckDB instance (in Python's case, the `duckdb` module).
...
Also note the rules on [identifiers and database names in particular]({% link docs/lts/sql/dialect/keywords_and_identifiers.md %}#database-names).
