...
Running `DELETE` does not mean space is reclaimed. In general, rows are only marked as deleted. DuckDB reclaims space upon [performing a `CHECKPOINT`]({% link docs/lts/sql/statements/checkpoint.md %}). [`VACUUM`]({% link docs/lts/sql/statements/vacuum.md %}) currently does not reclaim space.
