...
Temporary tables reside in memory rather than on disk even when connecting to a persistent DuckDB, but if the `temp_directory` [configuration]({% link docs/lts/configuration/overview.md %}) is set, data will be spilled to disk if memory becomes constrained.
...
This syntax can be used in combination with the [CSV reader]({% link docs/lts/data/csv/overview.md %}), the shorthand to read directly from CSV files without specifying a function, the [`FROM`-first syntax]({% link docs/lts/sql/query_syntax/from.md %}), and the [HTTP(S) support]({% link docs/lts/core_extensions/httpfs/https.md %}), yielding concise SQL commands such as the following:
