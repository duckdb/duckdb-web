...
When performing comparison on different types, DuckDB performs [Combination Casting]({% link docs/lts/sql/data_types/typecasting.md %}#combination-casting).
...
> It is not possible to enforce stricter type-checking for DuckDB's comparison operators. If you require stricter type-checking, consider creating a [macro]({% link docs/lts/sql/statements/create_macro.md %}) with the [`typeof` function]({% link docs/lts/sql/functions/utility.md %}#typeofexpression) or implementing a [user-defined function]({% link docs/lts/clients/python/function.md %}).
