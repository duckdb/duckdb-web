...
> The `TRY` expression was inspired by the [`TRY_CAST` expression]({% link docs/lts/sql/expressions/cast.md %}#try_cast).
...
`TRY` cannot be used in combination with a volatile function or with a [scalar subquery]({% link docs/lts/sql/expressions/subqueries.md %}#scalar-subquery).
For example:
