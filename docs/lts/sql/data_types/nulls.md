...
For details, see the [Boolean Type documentation]({% link docs/lts/sql/data_types/boolean.md %}).
...
For details, see the [`IN` documentation]({% link docs/lts/sql/expressions/in.md %}).

## `NULL` and Aggregate Functions

`NULL` values are ignored in most aggregate functions.

Aggregate functions that do not ignore `NULL` values include: `first`, `last`, `list` and `array_agg`. To exclude `NULL` values from those aggregate functions, the [`FILTER` clause]({% link docs/lts/sql/query_syntax/filter.md %}) can be used.
