...
`UNION` values can be created with the [`union_value(tag := expr)`]({% link docs/lts/sql/functions/union.md %}) function or by [casting from a member type](#casting-to-unions).
...
Alternatively, you can use 'dot syntax' similarly to [`STRUCT`s]({% link docs/lts/sql/data_types/struct.md %}).
...
Since `UNION` types are implemented on top of `STRUCT` types internally, they can be used with all the comparison operators as well as in both `WHERE` and `HAVING` clauses with the [same semantics as `STRUCT`s]({% link docs/lts/sql/data_types/struct.md %}#comparison-operators). The “tag” is always stored as the first struct entry, which ensures that the `UNION` types are compared and ordered by “tag” first.

## Functions

See [Union Functions]({% link docs/lts/sql/functions/union.md %}).
