...
Retrieving a value from a struct can be accomplished using dot notation, bracket notation, or through [struct functions]({% link docs/lts/sql/functions/struct.md %}) like `struct_extract`.
...
The same can be achieved with the star notation (`*`), which additionally allows [modifications of the returned columns]({% link docs/lts/sql/expressions/star.md %}):
...
The `STRUCT` type can be compared using all the [comparison operators]({% link docs/lts/sql/expressions/comparison_operators.md %}).
...
using the [`ALTER TABLE` clause]({% link docs/lts/sql/statements/alter_table.md %}).
...
See [Struct Functions]({% link docs/lts/sql/functions/struct.md %}).
