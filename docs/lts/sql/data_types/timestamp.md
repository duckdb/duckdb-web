...
Timestamps represent points in time. As such, they combine [`DATE`]({% link docs/lts/sql/data_types/date.md %}) and [`TIME`]({% link docs/lts/sql/data_types/time.md %}) information.
...
Despite the name, a `TIMESTAMP WITH TIME ZONE` does not store time zone information. Instead, it only stores the `INT64` number of non-leap microseconds since the Unix epoch `1970-01-01 00:00:00+00`, and thus unambiguously identifies a point in absolute time, or [*instant*]({% link docs/lts/sql/data_types/timestamp.md %}#instants). The reason for the labels *time zone aware* and `WITH TIME ZONE` is that timestamp arithmetic, [*binning*]({% link docs/lts/sql/data_types/timestamp.md %}#temporal-binning), and string formatting for this type are performed in a [configured time zone]({% link docs/lts/sql/data_types/timestamp.md %}#time-zone-support), which defaults to the system time zone and is just `UTC+00:00` in the examples above.
...
See [Timestamp Functions]({% link docs/lts/sql/functions/timestamp.md %}).
...
The built-in [ICU extension]({% link docs/lts/core_extensions/icu.md %}) implements all the binning and arithmetic functions using the
...
You can also find a reference table of [available time zones]({% link docs/lts/sql/data_types/timezones.md %}).

## Calendar Support

The [ICU extension]({% link docs/lts/core_extensions/icu.md %}) also supports non-Gregorian calendars using the `SET Calendar` command.
