...
The values of the grouping columns themselves are unchanged, and any other columns can be combined using an [aggregate function]({% link docs/lts/sql/functions/aggregates.md %}) (such as `count`, `sum`, `avg`, etc).
...
Using the [`GROUPING SETS`, `CUBE` or `ROLLUP` clauses]({% link docs/lts/sql/query_syntax/grouping_sets.md %}) it is possible to group along multiple dimensions.
See the [`GROUPING SETS`]({% link docs/lts/sql/query_syntax/grouping_sets.md %}) page for more information.
