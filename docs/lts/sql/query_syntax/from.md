...
Examples include [`read_csv`]({% link docs/lts/data/csv/overview.md %}#csv-functions), [`read_parquet`]({% link docs/lts/data/parquet/overview.md %}#read_parquet-function), [`range`]({% link docs/lts/sql/functions/list.md %}#rangestart-stop-step), [`generate_series`]({% link docs/lts/sql/functions/list.md %}#generate_seriesstart-stop-step), [`repeat`]({% link docs/lts/sql/functions/utility.md %}#repeat_rowvarargs-num_rows), [`unnest`]({% link docs/lts/sql/query_syntax/unnest.md %}), and [`glob`]({% link docs/lts/sql/functions/utility.md %}#globsearch_path) (note that some of the examples here can be used as both scalar and table functions). 
...
Note that the same result could be achieved using the [`row_number` window function]({% link docs/lts/sql/functions/window_functions.md %}#row_numberorder-by-ordering).
...
Semi joins provide the same logic as the [`IN` operator]({% link docs/lts/sql/expressions/in.md %}) statement.
...
relational tables are not ordered, but imported tables such as [data frames]({% link docs/lts/clients/python/data_ingestion.md %}#pandas-dataframes-–-object-columns)
or disk files (like [CSVs]({% link docs/lts/data/csv/overview.md %}) or [Parquet files]({% link docs/lts/data/parquet/overview.md %})) do have a natural ordering.
