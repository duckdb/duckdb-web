...
The `date_part`, `date_trunc` and `date_diff` functions can be used to extract or manipulate parts of temporal types such as [`TIMESTAMP`]({% link docs/lts/sql/data_types/timestamp.md %}), [`TIMESTAMPTZ`]({% link docs/lts/sql/data_types/timestamp.md %}), [`DATE`]({% link docs/lts/sql/data_types/date.md %}) and [`INTERVAL`]({% link docs/lts/sql/data_types/interval.md %}).
...
Note that the time zone parts are all zero unless a time zone extension such as [ICU]({% link docs/lts/core_extensions/icu.md %})
