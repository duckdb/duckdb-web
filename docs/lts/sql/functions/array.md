...
All [`LIST` functions]({% link docs/lts/sql/functions/list.md %}) work with the [`ARRAY` data type]({% link docs/lts/sql/data_types/array.md %}). Additionally, several `ARRAY`-native functions are also supported.
