...
The preferred way to configure and authenticate to S3 endpoints is to use [secrets]({% link docs/lts/sql/statements/create_secret.md %}). Multiple secret providers are available.

To migrate from the [deprecated S3 API]({% link docs/lts/core_extensions/httpfs/s3api_legacy_authentication.md %}), use a defined secret with a profile.
...
This approach is equivalent to the [deprecated S3 API's]({% link docs/lts/core_extensions/httpfs/s3api_legacy_authentication.md %})'s method `load_aws_credentials('⟨my_profile⟩')`.
...
The `httpfs` extension supports [partial reading]({% link docs/lts/core_extensions/httpfs/https.md %}#partial-reading) from S3 buckets.
...
File [globbing]({% link docs/lts/sql/functions/pattern_matching.md %}#globbing) is implemented using the ListObjectsV2 API call and allows using filesystem-like glob patterns to match multiple files, for example:
...
This query matches all files in the root of the bucket with the [Parquet extension]({% link docs/lts/data/parquet/overview.md %}).
...
DuckDB also offers support for the [Hive partitioning scheme]({% link docs/lts/data/partitioning/hive_partitioning.md %}), which is available when using HTTP(S) and S3 endpoints.
