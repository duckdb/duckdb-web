...
- /docs/lts/extensions/httpfs/s3api_legacy_authentication
title: Legacy Authentication Scheme for S3 API
---

Prior to version 0.10.0, DuckDB did not have a [Secrets manager]({% link docs/lts/sql/statements/create_secret.md %}). Hence, the configuration of and authentication to S3 endpoints was handled via variables. This page documents the legacy authentication scheme for the S3 API.
...
> The recommended way to configure and authenticate of S3 endpoints is to use [secrets]({% link docs/lts/core_extensions/httpfs/s3api.md %}#configuration-and-authentication).
...
The [`aws` extension]({% link docs/lts/core_extensions/aws.md %}) allows for loading AWS credentials.
