...
The `autocomplete` extension adds support for autocomplete in the [CLI client]({% link docs/lts/clients/cli/overview.md %}).
The extension is shipped by default with the CLI client.

## Behavior

For the behavior of the `autocomplete` extension, see the [documentation of the CLI client]({% link docs/lts/clients/cli/autocomplete.md %}).
