...
> Tip Previously, reading and writing Excel files was handled through the [`spatial` extension]({% link docs/lts/core_extensions/spatial/overview.md %}), which coincidentally included support for XLSX files through one of its dependencies, but this capability may be removed from the `spatial` extension. Additionally, the `excel` extension is more efficient and provides more control over the import/export process. If the `excel` extension is insufficient for your use case, try using the [`spatial` extension]({% link docs/lts/core_extensions/spatial/overview.md %}). See the [Excel Import]({% link docs/lts/guides/file_formats/excel_import.md %}) and [Excel Export]({% link docs/lts/guides/file_formats/excel_export.md %}) pages for instructions. However, please be aware that these features may be deprecated in the future.

## Installing and Loading

The `excel` extension will be transparently [autoloaded]({% link docs/lts/extensions/overview.md %}#autoloading-extensions) on first use from the official extension repository.
