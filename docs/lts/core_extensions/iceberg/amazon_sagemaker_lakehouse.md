...
> follow the [extension documentation]({% link docs/lts/extensions/installing_extensions.md %}#force-installing-to-upgrade-extensions).

## Connecting to Amazon SageMaker Lakehouse (AWS Glue)

Create an S3 secret using the [Secrets Manager]({% link docs/lts/configuration/secrets_manager.md %}):
...
In this example we use an STS token, but [other authentication methods are supported]({% link docs/lts/core_extensions/aws.md %}).
