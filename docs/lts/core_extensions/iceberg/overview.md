...
In this page we will go over the basic usage of the extension without the need to attach to an Iceberg catalog. For full support – including write support – see [how to attach Iceberg REST catalogs]({% link docs/lts/core_extensions/iceberg/iceberg_rest_catalogs.md %}).
...
To make sure that you have the latest version, [update your extensions]({% link docs/lts/sql/statements/update_extensions.md %}):
...
The `iceberg` extension works together with the [`httpfs` extension]({% link docs/lts/core_extensions/httpfs/overview.md %}) or the [`azure` extension]({% link docs/lts/core_extensions/azure.md %}) to access Iceberg tables in object stores such as S3 or Azure Blob Storage.
...
For a set of unsupported operations when attaching to an Iceberg catalog, see [Unsupported Operations]({% link docs/lts/core_extensions/iceberg/iceberg_rest_catalogs.md %}#unsupported-operations).
