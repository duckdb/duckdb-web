...
Check [`EXPORT` documentation]({% link docs/lts/sql/statements/export.md %}) for more details on the syntax.
...
The `storage_compatibility_version` [configuration option]({% link docs/lts/configuration/overview.md %}#configuration-reference) can also be used to specify the storage version to use. It can be specified in various ways, for example at connect time using the Python bindings it looks as follows:

```python
duckdb.connect("file.db", config={'storage_compatibility_version': 'latest'})
```

When using the [commandline client]({% link docs/lts/clients/cli/overview.md %}), the storage version can be specified using the `-storage-version` option.
...
To turn on compression for in-memory databases, use `ATTACH` with the [`COMPRESS` option]({% link docs/lts/sql/statements/attach.md %}#options).
...
Several features in DuckDB, including [parallelism]({% link docs/lts/guides/performance/how_to_tune_workloads.md %}) and [compression]({% post_url 2022-10-28-lightweight-compression %}) are based on row groups.
...
2. Open the database with the latest version of DuckDB, export it to a standard format (e.g., Parquet), then import it to any version of DuckDB. See the [`EXPORT/IMPORT DATABASE` statements]({% link docs/lts/sql/statements/export.md %}) for details.
