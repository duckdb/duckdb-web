...
For platforms where packages for certain extensions are not available, users can build them from source and [install the resulting binaries manually]({% link docs/lts/extensions/advanced_installation_methods.md %}#installing-an-extension-from-an-explicit-path).
...
* `windows_amd64_mingw`
* `wasm_eh` and `wasm_mvp` (see [DuckDB-Wasm's extensions]({% link docs/lts/clients/wasm/extensions.md %}))
...
which itself is supported through an extension ([`httpfs`]({% link docs/lts/core_extensions/httpfs/overview.md %})).

### Unsigned Extensions

> Warning Only load unsigned extensions from sources you trust.
> Avoid loading unsigned extensions over HTTP.
> Consult the [Securing DuckDB page]({% link docs/lts/operations_manual/securing_duckdb/securing_extensions.md %}) for guidelines on how to set up DuckDB in a secure manner.
...
To load unsigned extensions using the [CLI client]({% link docs/lts/clients/cli/overview.md %}), pass the `-unsigned` flag to it on startup:
...
For client APIs, the `allow_unsigned_extensions` database configuration options needs to be set, see the respective [Client API docs]({% link docs/lts/clients/overview.md %}).
For example, for the Python client, see the [Loading and Installing Extensions section in the Python API documentation]({% link docs/lts/clients/python/overview.md %}#loading-and-installing-extensions).
...
This structure is described in the [“Downloading Extensions Directly from S3” section]({% link docs/lts/extensions/advanced_installation_methods.md %}#downloading-extensions-directly-from-s3), and is the same
...
If the custom repository is served over HTTPS or S3, the [`httpfs` extension]({% link docs/lts/core_extensions/httpfs/overview.md %}) is required. DuckDB will attempt to [autoload]({% link docs/lts/extensions/overview.md %}#autoloading-extensions)
