...
API to install and load extensions. For example, the [Python client]({% link docs/lts/clients/python/overview.md %}#loading-and-installing-extensions), has dedicated `install_extension(name: str)` and `load_extension(name: str)` methods. For more details on a specific client API, refer
to the [Client API documentation]({% link docs/lts/clients/overview.md %})
...
* Extensions cannot be reloaded. If you [update extensions]({% link docs/lts/sql/statements/update_extensions.md %}), restart the DuckDB process to use newer extensions.
