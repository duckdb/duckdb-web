...
If you plan to embed DuckDB in your application, please consult the [“Embedding DuckDB”]({% link docs/1.3/operations_manual/securing_duckdb/embedding_duckdb.md %}) page.
...
DuckDB's CLI client supports [“safe mode”]({% link docs/1.3/clients/cli/safe_mode.md %}), which prevents DuckDB from accessing external files other than the database file.
This can be activated via a command line argument or a [dot command]({% link docs/1.3/clients/cli/dot_commands.md %}):
...
DuckDB can list directories and read arbitrary files via its CSV parser’s [`read_csv` function]({% link docs/1.3/data/csv/overview.md %}) or read text via the [`read_text` function]({% link docs/1.3/sql/functions/text.md %}#read_textsource).
...
Second, you can also completely disable external access by setting the [`enable_external_access` option]({% link docs/1.3/configuration/overview.md %}#configuration-reference) option to `false`.
...
[Persistent secrets]({% link docs/1.3/configuration/secrets_manager.md %}#persistent-secrets) are stored in unencrypted binary format on the disk. These have the same permissions as SSH keys, `600`, i.e., only user who is running the DuckDB (parent) process can read and write them.
...
Similarly to other SQL databases, it's recommended to use [prepared statements]({% link docs/1.3/sql/query_syntax/prepared_statements.md %}) in DuckDB to prevent [SQL injection](https://en.wikipedia.org/wiki/SQL_injection).
...
This introduces security considerations. Therefore, we recommend reviewing the configuration options for [securing extensions]({% link docs/1.3/operations_manual/securing_duckdb/securing_extensions.md %}).
