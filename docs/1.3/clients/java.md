...
settings can be changed later on using [`PRAGMA` statements]({% link docs/1.3/configuration/pragmas.md %}) as well.
...
The [Appender]({% link docs/1.3/data/appender.md %}) is available in the DuckDB JDBC driver via the `org.duckdb.DuckDBAppender` class.
