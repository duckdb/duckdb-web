...
> Please use the [DuckDB Node Neo package]({% link docs/1.3/clients/node_neo/overview.md %}) instead.
...
All options as described on [Database configuration]({% link docs/1.3/configuration/overview.md %}#configuration-reference) can be (optionally) supplied to the `Database` constructor as second argument. The third argument can be optionally supplied to get feedback on the given options.
...
[Apache Arrow]({% link docs/1.3/guides/python/sql_on_arrow.md %}) can be used to insert data into DuckDB without making a copy:
...
To load [unsigned extensions]({% link docs/1.3/core_extensions/overview.md %}#unsigned-extensions), instantiate the database as follows:
