...
To make sure that you have the latest version, [update your extensions]({% link docs/1.3/sql/statements/update_extensions.md %}):
...
The `iceberg` works together with the [`httpfs` extension]({% link docs/1.3/core_extensions/httpfs/overview.md %}) or the [`azure` extension]({% link docs/1.3/core_extensions/azure.md %}) to access Iceberg tables in object stores such as S3 or Azure Blob Storage.
