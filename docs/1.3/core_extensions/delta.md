...
The `delta` extension will be transparently [autoloaded]({% link docs/1.3/core_extensions/overview.md %}#autoloading-extensions) on first use from the official extension repository.
...
To scan a Delta table in an [S3 bucket]({% link docs/1.3/core_extensions/httpfs/s3api.md %}), run:
...
For authenticating to S3 buckets, DuckDB [Secrets]({% link docs/1.3/configuration/secrets_manager.md %}) are supported:
...
To scan a Delta table in an [Azure Blob Storage bucket]({% link docs/1.3/core_extensions/azure.md %}#azure-blob-storage), run:
...
For authenticating to Azure Blob Storage, DuckDB [Secrets]({% link docs/1.3/configuration/secrets_manager.md %}) are supported:
...
Support for the [other DuckDB platforms]({% link docs/1.3/extensions/extension_distribution.md %}#platforms) is work-in-progress.
