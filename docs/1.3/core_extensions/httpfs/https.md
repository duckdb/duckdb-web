...
For Parquet files, DuckDB supports [partial reading]({% link docs/1.3/data/parquet/overview.md %}#partial-reading), i.e., it can use a combination of the Parquet metadata and [HTTP range requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/Range_requests) to only download the parts of the file that are actually required by the query. For example, the following query will only read the Parquet metadata and the data for the `column_a` column:
...
To authenticate for an HTTP(S) endpoint, create an `HTTP` secret using the [Secrets Manager]({% link docs/1.3/configuration/secrets_manager.md %}):
...
You can add an HTTP proxy using the [Secrets Manager]({% link docs/1.3/configuration/secrets_manager.md %}):
...
Alternatively, you can add it via [configuration options]({% link docs/1.3/configuration/pragmas.md %}):
...
To use the `httpfs` extension with a custom certificate file, set the following [configuration options]({% link docs/1.3/configuration/pragmas.md %}) prior to loading the extension:
