...
Running `DELETE` does not mean space is reclaimed. In general, rows are only marked as deleted. DuckDB reclaims space upon [performing a `CHECKPOINT`]({% link docs/1.3/sql/statements/checkpoint.md %}). [`VACUUM`]({% link docs/1.3/sql/statements/vacuum.md %}) currently does not reclaim space.
