...
There are two options to change the sub-schema of a [`STRUCT`]({% link docs/1.3/sql/data_types/struct.md %})-typed column.
...
[`ADD COLUMN`, `DROP COLUMN` and `RENAME COLUMN` clauses]({% link docs/1.3/sql/data_types/struct.md %}#updating-the-schema)
