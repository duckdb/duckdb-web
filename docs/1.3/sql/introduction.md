...
> The few exceptions to this are listed on the [PostgreSQL compatibility page]({% link docs/1.3/sql/dialect/postgresql_compatibility.md %}).
...
Where the file name for the source file must be available on the machine running the process. There are many other ways of loading data into DuckDB, see the [corresponding documentation section]({% link docs/1.3/data/overview.md %}) for more information.
...
More information about the `LIKE` operator can be found in the [pattern matching page]({% link docs/1.3/sql/functions/pattern_matching.md %}).
