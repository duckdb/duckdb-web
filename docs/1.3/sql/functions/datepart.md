...
The `date_part` and `date_diff` and `date_trunc` functions can be used to manipulate the fields of temporal types such as [`DATE`]({% link docs/1.3/sql/data_types/date.md %}) and [`TIMESTAMP`]({% link docs/1.3/sql/data_types/timestamp.md %}).
...
Note that the time zone parts are all zero unless a time zone extension such as [ICU]({% link docs/1.3/core_extensions/icu.md %})
