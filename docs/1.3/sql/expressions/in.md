...
The `IN` operator works on [maps]({% link docs/1.3/sql/data_types/map.md %}) according to the semantics used in Python, i.e., it checks for the presence of keys (not values):
...
The `IN` operator works with [subqueries]({% link docs/1.3/sql/expressions/subqueries.md %}) that return a single column.
For example:
...
The `IN` operator can be used as a shorthand for the [`contains` string function]({% link docs/1.3/sql/functions/text.md %}#containsstring-search_string).
For example:
