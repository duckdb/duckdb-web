...
Below is an example of `SUMMARIZE` on the `lineitem` table of TPC-H `SF1` table, generated using the [`tpch` extension]({% link docs/1.3/core_extensions/tpch.md %}).
...
It is possible to summarize remote tables via the [`httpfs` extension]({% link docs/1.3/core_extensions/httpfs/overview.md %}) using the `SUMMARIZE TABLE` statement. For example:
