...
The [`query`]({% link docs/1.3/sql/functions/utility.md %}#queryquery_string_literal)
and [`query_table`]({% link docs/1.3/sql/functions/utility.md %}#query_tabletbl_name)
...
When combined with the [`COLUMNS` expression]({% link docs/1.3/sql/expressions/star.md %}#columns), we can write very generic SQL-only macros. For example, below is a custom version of `SUMMARIZE` that computes the `min` and `max` of every column in a table:
