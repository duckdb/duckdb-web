...
We can use the [`FROM`-first syntax]({% link docs/1.3/sql/query_syntax/from.md %}#from-first-syntax) to omit `SELECT *`.
...
| `auto_detect` | [Auto detect CSV parameters]({% link docs/1.3/data/csv/auto_detection.md %}). | `BOOL` | `true` |
...
| `dateformat` | [Date format]({% link docs/1.3/sql/functions/dateformat.md %}) used when parsing and writing dates. | `VARCHAR` | (empty) |
...
| `hive_partitioning` | Interpret the path as a [Hive partitioned path]({% link docs/1.3/data/partitioning/hive_partitioning.md %}). | `BOOL` | (auto-detected) |
