...
[Extensions]({% link docs/1.1/extensions/overview.md %}) can be built from source and installed from the resulting local binary.
...
This option also accepts out-of-tree extensions such as [`delta`]({% link docs/1.1/extensions/delta.md %}):
...
When this flag is set, the [`jemalloc` extension]({% link docs/1.1/extensions/jemalloc.md %}) is built.
