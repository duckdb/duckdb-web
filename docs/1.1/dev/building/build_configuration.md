...
[Core DuckDB extensions]({% link docs/1.1/extensions/core_extensions.md %}) are the ones maintaned by the DuckDB team. These are hosted in the `duckdb` GitHub organization and are served by the `core` extension repository.
...
More on this topic at [building duckdb extensions]({% link docs/1.1/dev/building/building_extensions.md %}).
...
When this flag is set, the [Python]({% link docs/1.1/api/python/overview.md %}) package is built.

### `BUILD_SHELL`

When this flag is set, the [CLI]({% link docs/1.1/api/cli/overview.md %}) is built, this is usually enabled by default.
...
When this flag is set, the [Java]({% link docs/1.1/api/java.md %}) package is built.

### `BUILD_ODBC`

When this flag is set, the [ODBC]({% link docs/1.1/api/odbc/overview.md %}) package is built.
