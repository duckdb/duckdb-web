...
To load an Iceberg file from S3, both the [`httpfs`]({% link docs/1.1/extensions/httpfs/overview.md %}) and [`iceberg`]({% link docs/1.1/extensions/iceberg.md %}) extensions are required. They can be installed using the `INSTALL` SQL command. The extensions only need to be installed once.
