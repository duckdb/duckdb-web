...
layout: docu
redirect_from:
- /docs/archive/1.1/guides/import/s3_express_one
...
DuckDB can read S3 Express One buckets using the [`httpfs` extension]({% link docs/1.1/extensions/httpfs/overview.md %}).

## Credentials and Configuration

The configuration of S3 Express One buckets is similar to [regular S3 buckets]({% link docs/1.1/guides/network_cloud_storage/s3_import.md %}) with one exception:
...
For example, to allow DuckDB to use an S3 Express One bucket, configure the [Secrets manager]({% link docs/1.1/sql/statements/create_secret.md %}) as follows:
...
We ran two experiments on a `c7gd.12xlarge` instance using the [LDBC SF300 Comments `creationDate` Parquet file](https://blobs.duckdb.org/data/ldbc-sf300-comments-creationDate.parquet) file (also used in the [microbenchmarks of the performance guide]({% link docs/1.1/guides/performance/benchmarks.md %}#data-sets)).
...
The “loading only” variant is running the load as part of an [`EXPLAIN ANALYZE`]({% link docs/1.1/guides/meta/explain_analyze.md %}) statement to measure the runtime without account creating a local table, while the “creating local table” variant uses [`CREATE TABLE ... AS SELECT`]({% link docs/1.1/sql/statements/create_table.md %}#create-table--as-select-ctas) to create a persistent table on the local disk.