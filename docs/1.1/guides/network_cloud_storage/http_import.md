...
To load a Parquet file over HTTP(S), the [`httpfs` extension]({% link docs/1.1/extensions/httpfs/overview.md %}) is required. This can be installed using the `INSTALL` SQL command. This only needs to be run once.
...
Moreover, the `read_parquet` function itself can also be omitted thanks to DuckDB's [replacement scan mechanism]({% link docs/1.1/api/c/replacement_scans.md %}):
