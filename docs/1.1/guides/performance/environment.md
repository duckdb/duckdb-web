...
If you are working in a memory-constrained environment, consider [limiting the number of threads]({% link docs/1.1/configuration/pragmas.md %}#threads), e.g., by issuing:
...
Read more in the [“How to Tune Workloads” page]({% link docs/1.1/guides/performance/how_to_tune_workloads.md %}#persistent-vs-in-memory-tables).
...
If you have a many-core CPU running on a system where DuckDB ships with [`jemalloc`]({% link docs/1.1/extensions/jemalloc.md %}) as the default memory allocator, consider [enabling the allocator's background threads]({% link docs/1.1/extensions/jemalloc.md %}#background-threads).