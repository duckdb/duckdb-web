...
When loading CSV files, you may leverage the CSV reader's [auto-detection mechanism]({% link docs/1.1/data/csv/auto_detection.md %}) to get the correct types for CSV inputs.
...
First, we use a `DATETIME` to encode the values and run the query using the [`extract` datetime function]({% link docs/1.1/sql/functions/timestamp.md %}):
