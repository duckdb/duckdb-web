...
|---------------|-----------------------|---------------------------------------------------------------|
| `AZURE`       | Azure Blob Storage    | [`azure`]({% link docs/1.1/extensions/azure.md %})                |
| `GCS`         | Google Cloud Storage  | [`httpfs`]({% link docs/1.1/extensions/httpfs/s3api.md %})        |
| `HTTP`        | HTTP and HTTPS        | [`httpfs`]({% link docs/1.1/extensions/httpfs/https.md %})        |
| `HUGGINGFACE` | Hugging Face          | [`httpfs`]({% link docs/1.1/extensions/httpfs/hugging_face.md %}) |
| `MYSQL`       | MySQL                 | [`mysql`]({% link docs/1.1/extensions/mysql.md %})                |
| `POSTGRES`    | PostgreSQL            | [`postgres`]({% link docs/1.1/extensions/postgres.md %})          |
| `R2`          | Cloudflare R2         | [`httpfs`]({% link docs/1.1/extensions/httpfs/s3api.md %})        |
| `S3`          | AWS S3                | [`httpfs`]({% link docs/1.1/extensions/httpfs/s3api.md %})        |

For each type, there are one or more “secret providers” that specify how the secret is created. Secrets can also have an optional scope, which is a file path prefix that the secret applies to. When fetching a secret for a path, the secret scopes are compared to the path, returning the matching secret for the path. In the case of multiple matching secrets, the longest prefix is chosen.

## Creating a Secret

Secrets can be created using the [`CREATE SECRET` SQL statement]({% link docs/1.1/sql/statements/create_secret.md %}).
...
Secrets can be deleted using the [`DROP SECRET` statement]({% link docs/1.1/sql/statements/create_secret.md %}#syntax-for-drop-secret), e.g.:
...
Secrets can be listed using the built-in table-producing function, e.g., by using the [`duckdb_secrets()` table function]({% link docs/1.1/sql/meta/duckdb_table_functions.md %}#duckdb_secrets):
