...
Note that the arrow operator `->`, which is used for JSON extracts, has a low precedence as it is also used in [lambda functions]({% link docs/1.1/sql/functions/lambda.md %}).
...
> Warning DuckDB's JSON data type uses [0-based indexing]({% link docs/1.1/data/json/overview.md %}#indexing).
...
Note that DuckDB's JSON data type uses [0-based indexing]({% link docs/1.1/data/json/overview.md %}#indexing).
