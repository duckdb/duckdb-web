...
DuckDB implements a custom C API modelled somewhat following the SQLite C API. The API is contained in the `duckdb.h` header. Continue to [Startup & Shutdown]({% link docs/1.1/api/c/connect.md %}) to get started, or check out the [Full API overview]({% link docs/1.1/api/c/api.md %}).
