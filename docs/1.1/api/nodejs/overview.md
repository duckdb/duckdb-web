...
> Please use the [DuckDB Node Neo package]({% link docs/1.1/api/node_neo/overview.md %}) instead.
...
All options as described on [Database configuration]({% link docs/1.1/configuration/overview.md %}#configuration-reference) can be (optionally) supplied to the `Database` constructor as second argument. The third argument can be optionally supplied to get feedback on the given options.
...
[Apache Arrow]({% link docs/1.1/guides/python/sql_on_arrow.md %}) can be used to insert data into DuckDB without making a copy:
...
To load [unsigned extensions]({% link docs/1.1/extensions/overview.md %}#ensuring-the-integrity-of-extensions), instantiate the database as follows:
