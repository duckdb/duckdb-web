...
The values of the grouping columns themselves are unchanged, and any other columns can be combined using an [aggregate function]({% link docs/1.1/sql/functions/aggregates.md %}) (such as `count`, `sum`, `avg`, etc).
...
Using the [`GROUPING SETS`, `CUBE` or `ROLLUP` clauses]({% link docs/1.1/sql/query_syntax/grouping_sets.md %}) it is possible to group along multiple dimensions.
See the [`GROUPING SETS`]({% link docs/1.1/sql/query_syntax/grouping_sets.md %}) page for more information.
