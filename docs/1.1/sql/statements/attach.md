...
Attach a SQLite database for reading and writing (see the [`sqlite` extension]({% link docs/1.1/extensions/sqlite.md %}) for more information):
...
> Warning Closing the connection, e.g., invoking the [`close()` function in Python]({% link docs/1.1/api/python/dbapi.md %}#connection), does not release the locks held on the database files as the file handles are held by the main DuckDB instance (in Python's case, the `duckdb` module).
...
Also note the rules on [identifiers and database names in particular]({% link docs/1.1/sql/dialect/keywords_and_identifiers.md %}#database-names).
