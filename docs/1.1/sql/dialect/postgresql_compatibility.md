...
Therefore, DuckDB achieves case insensitivity by making identifiers fully case insensitive throughout the system but [_preserving their case_]({% link docs/1.1/sql/dialect/keywords_and_identifiers.md %}#rules-for-case-sensitivity).
...
PostgreSQL's behavior of lowercasing identifiers is accessible using the [`preserve_identifier_case` option]({% link docs/1.1/configuration/overview.md %}#local-configuration-options):
...
In DuckDB, the [`VACUUM` statement]({% link docs/1.1/sql/statements/vacuum.md %}) is only used to rebuild statistics.
...
Instead, please use the [`strptime` function]({% link docs/1.1/sql/functions/dateformat.md %}#strptime-examples).
...
For [`CREATE TABLE` statements]({% link docs/1.1/sql/statements/create_table.md %}), DuckDB attempts to resolve type names in the schema where a table is created. For example:
