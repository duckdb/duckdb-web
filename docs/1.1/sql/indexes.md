...
DuckDB has two built-in index types. Indexes can also be defined via [extensions]({% link docs/1.1/extensions/overview.md %}).

### Min-Max Index (Zonemap)

A [min-max index](https://en.wikipedia.org/wiki/Block_Range_Index) (also known as zonemap and block range index) is automatically created for columns of all [general-purpose data types]({% link docs/1.1/sql/data_types/overview.md %}).
...
[Starting with version 1.1.0]({% post_url 2024-09-09-announcing-duckdb-110 %}#r-tree), DuckDB supports [R-trees for spatial indexing]({% link docs/1.1/extensions/spatial/r-tree_indexes.md %}) via the `spatial` extension.
...
To create an index, use the [`CREATE INDEX` statement]({% link docs/1.1/sql/statements/create_index.md %}#create-index).
To drop an index, use the [`DROP INDEX` statement]({% link docs/1.1/sql/statements/create_index.md %}#drop-index).
...
> As expected, indexes have a strong effect on performance, slowing down loading and updates, but speeding up certain queries. Please consult the [Performance Guide]({% link docs/1.1/guides/performance/indexing.md %}) for details.
