...
* `windows_amd64_mingw`
* `wasm_eh` and `wasm_mvp` (see [DuckDB-Wasm's extensions]({% link docs/1.1/api/wasm/extensions.md %}))
...
If the custom repository is served over HTTPS or S3, the [`httpfs` extension]({% link docs/1.1/extensions/httpfs/overview.md %}) is required. DuckDB will attempt to [autoload]({% link docs/1.1/extensions/overview.md %}#autoloading-extensions)
...
For building and installing extensions from source, see the [building guide]({% link docs/1.1/dev/building/overview.md %}).
...
* core out-of tree extensions tend to live in a repository in `github.com/duckdb/duckdb_⟨ext_name⟩` but the name may vary. See the [full list]({% link docs/1.1/extensions/core_extensions.md %}) of core extensions for details.
