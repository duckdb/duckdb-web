...
### Connecing with IAM authentication from command line

> Note, this section illustrates the process with `psql` utility, not with DuckDB. This method can be used to check the configuration before setting up DuckDB secrets.
...
```sql
ATTACH 'postgres:' as p1 (SECRET pg_rds_secret1, SECRET_STORAGE_TABLE duckdb_secrets);
...
When `IN postgres_<attached_database>` clause of `CREATE SECRET` is specified, the secret is persisted to the `duckdb_secrets` table in the specified attached database.
