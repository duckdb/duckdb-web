...
For plain HTTP(S), only file reading is supported. For object storage using the S3 API, the `httpfs` extension supports reading/writing/[globbing]({% link docs/current/sql/functions/pattern_matching.md %}#globbing) files.
...
The `httpfs` extension supports connecting to [HTTP(S) endpoints]({% link docs/current/core_extensions/httpfs/https.md %}).

## S3 API

The `httpfs` extension supports connecting to [S3 API endpoints]({% link docs/current/core_extensions/httpfs/s3api.md %}).
