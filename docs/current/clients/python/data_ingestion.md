...
See the [CSV Import]({% link docs/current/data/csv/overview.md %}) page for more information.
...
Read a Parquet file over [https]({% link docs/current/core_extensions/httpfs/overview.md %}):
...
See the [Parquet Loading]({% link docs/current/data/parquet/overview.md %}) page for more information.
...
These types include the following: Pandas DataFrame, Polars DataFrame, Polars LazyFrame, NumPy arrays, [relations]({% link docs/current/clients/python/relational_api.md %}) and Arrow objects.

Only variables that are visible to Python code at the location of the `sql()` or `execute()` call can be used in this manner.
Accessing these variables is made possible by [replacement scans]({% link docs/current/clients/c/replacement_scans.md %}). To disable replacement scans entirely, use:
