...
In Python, the output of the [`EXPLAIN` statement]({% link docs/current/guides/meta/explain.md %}) contains hard line breaks (`\n`):
...
Please also check out the [Jupyter guide]({% link docs/current/guides/python/jupyter.md %}) for tips on using Jupyter with JupySQL.
...
Passing query parameters to the [`sql()`]({% link docs/current/clients/python/relational_api.md %}#sql), [`query()`]({% link docs/current/clients/python/relational_api.md %}#query), or [`from_query()`]({% link docs/current/clients/python/relational_api.md %}#from_query) methods has significant performance overhead.
...
Instead, use [`execute()`]({% link docs/current/clients/python/dbapi.md %}#prepared-statements) for the parameterized query, then feed the result into the relational API via a [replacement scan]({% link docs/current/clients/python/relational_api.md %}#sql):
