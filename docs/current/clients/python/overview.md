...
DuckDB can ingest data from a wide variety of formats – both on-disk and in-memory. See the [data ingestion page]({% link docs/current/clients/python/data_ingestion.md %}) for more information.
...
Note that these are read-only, i.e., editing these tables via [`INSERT`]({% link docs/current/sql/statements/insert.md %}) or [`UPDATE` statements]({% link docs/current/sql/statements/update.md %}) is not possible.
...
DuckDB supports converting query results efficiently to a variety of formats. See the [result conversion page]({% link docs/current/clients/python/conversion.md %}) for more information.
...
DuckDB supports writing Relation objects directly to disk in a variety of formats. The [`COPY` statement]({% link docs/current/sql/statements/copy.md %}) can be used to write data to disk using SQL as an alternative.
...
The `duckdb.connect()` accepts a `config` dictionary, where [configuration options]({% link docs/current/configuration/overview.md %}#configuration-reference) can be specified. For example:
...
To specify the [storage version]({% link docs/current/internals/storage.md %}), pass the `storage_compatibility_version` option:
...
To load [unsigned extensions]({% link docs/current/extensions/overview.md %}#unsigned-extensions), use:
...
> Consult the [Securing DuckDB page]({% link docs/current/operations_manual/securing_duckdb/securing_extensions.md %}) for guidelines on how set up DuckDB in a secure manner.
