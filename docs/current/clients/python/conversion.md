...
This is a mapping of Python object types to DuckDB [Logical Types]({% link docs/current/sql/data_types/overview.md %}):
...
If the dict is returned by a [function]({% link docs/current/clients/python/function.md %}), 
...
If the dict is returned by a [function]({% link docs/current/clients/python/function.md %}), 
the function will return a `MAP`, due to [automatic conversion]({% link docs/current/clients/python/types.md %}#dictkey_type-value_type).
...
Below are some examples using this functionality. See the [Python guides]({% link docs/current/guides/overview.md %}#python-client) for more examples.
