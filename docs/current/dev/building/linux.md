...
For different build configurations (`debug`, `relassert`, etc.), please consult the [“Build Configurations” page]({% link docs/current/dev/building/build_configuration.md %}).
...
**Solution:**
Create or edit the `~/.R/Makevars` file. This example also contains the [`MAKEFLAGS` setting to parallelize the build]({% link docs/current/dev/building/r.md %}#the-build-only-uses-a-single-thread ):
...
**Problem:**
When building the [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}) on Linux, the build may fail with the following error.
