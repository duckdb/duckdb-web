...
[Core DuckDB extensions]({% link docs/current/core_extensions/overview.md %}) are the ones maintained by the DuckDB team. These are hosted in the `duckdb` GitHub organization and are served by the `core` extension repository.
...
More on this topic at [building DuckDB extensions]({% link docs/current/dev/building/building_extensions.md %}).
...
When this flag is set, the [Python]({% link docs/current/clients/python/overview.md %}) package is built.

### `BUILD_SHELL`

When this flag is set, the [CLI]({% link docs/current/clients/cli/overview.md %}) is built, this is usually enabled by default.
...
When this flag is set, the [ODBC]({% link docs/current/clients/odbc/overview.md %}) package is built.
