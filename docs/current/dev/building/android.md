...
1. If you would like to use the [Ninja build system]({% link docs/current/dev/building/overview.md %}#prerequisites), make sure it is installed and available on the `PATH`.
...
1. To build the [command line client]({% link docs/current/clients/cli/overview.md %}) in the [Termux application](https://termux.dev/), install the following packages:
