...
To turn on compression for in-memory databases, use `ATTACH` with the [`COMPRESS` option]({% link docs/current/sql/statements/attach.md %}#options).
...
Several features in DuckDB, including [parallelism]({% link docs/current/guides/performance/how_to_tune_workloads.md %}) and [compression]({% post_url 2022-10-28-lightweight-compression %}) are based on row groups.
...
2. Open the database with the latest version of DuckDB, export it to a standard format (e.g., Parquet), then import it to any version of DuckDB. See the [`EXPORT/IMPORT DATABASE` statements]({% link docs/current/sql/statements/export.md %}) for details.
