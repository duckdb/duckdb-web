...
For platforms where packages for certain extensions are not available, users can build them from source and [install the resulting binaries manually]({% link docs/current/extensions/advanced_installation_methods.md %}#installing-an-extension-from-an-explicit-path).
...
* `windows_amd64_mingw`
* `wasm_eh` and `wasm_mvp` (see [DuckDB-Wasm's extensions]({% link docs/current/clients/wasm/extensions.md %}))
...
which itself is supported through an extension ([`httpfs`]({% link docs/current/core_extensions/httpfs/overview.md %})).
...
This structure is described in the [“Downloading Extensions Directly from S3” section]({% link docs/current/extensions/advanced_installation_methods.md %}#downloading-extensions-directly-from-s3), and is the same
...
If the custom repository is served over HTTPS or S3, the [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}) is required. DuckDB will attempt to [autoload]({% link docs/current/extensions/overview.md %}#autoloading-extensions)
