...
- /docs/preview/extensions/troubleshooting
- /docs/stable/extensions/troubleshooting
...
* the extension is available, but in a different repository, try for example `INSTALL ⟨name⟩ FROM core;`{:.language-sql .highlight} or `INSTALL ⟨name⟩ FROM community;`{:.language-sql .highlight} or `INSTALL ⟨name⟩ FROM core_nightly;`{:.language-sql .highlight} (see the [Installing Extensions page]({% link docs/current/extensions/installing_extensions.md %}#extension-repositories)).
