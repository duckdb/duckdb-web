...
The list includes most, but not all, available core extensions. For the full list, we maintain a [list of core extensions]({% link docs/current/core_extensions/overview.md %}).
...
For example, to use the built-in [`json` extension]({% link docs/current/data/json/overview.md %}) to read a JSON file:
...
> Extension installation and loading are subject to a few [limitations]({% link docs/current/extensions/installing_extensions.md %}#limitations).
...
to install and load the [`spatial` extension]({% link docs/current/core_extensions/spatial/overview.md %}), run:
...
DuckDB will automatically install and load the [`httpfs`]({% link docs/current/core_extensions/httpfs/overview.md %}) extension. No explicit `INSTALL` or `LOAD` statements are required.
...
To see which extensions can be autoloaded, check the [core extensions list]({% link docs/current/core_extensions/overview.md %}).
...
For more details on extension versions, refer to the [Extension Versioning page]({% link docs/current/extensions/versioning_of_extensions.md %}).
...
See the [installation instructions]({% link docs/current/extensions/installing_extensions.md %}) and the [advanced installation methods page]({% link docs/current/extensions/advanced_installation_methods.md %}).
