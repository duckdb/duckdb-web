...
- /docs/stable/extensions/versioning_of_extensions
...
of these version numbers across the various extensions, DuckDB's [Core Extensions]({% link docs/current/core_extensions/overview.md %}) use
...
Finally, there is a small exception: All [in-tree]({% link docs/current/extensions/advanced_installation_methods.md %}#in-tree-vs-out-of-tree) extensions simply
...
To achieve this, Core Extensions tend to regularly push builds to the [`core_nightly` repository]({% link docs/current/extensions/installing_extensions.md %}#extension-repositories). Let's look at an example:
