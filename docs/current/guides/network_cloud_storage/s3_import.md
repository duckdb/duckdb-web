...
To load a Parquet file from S3, the [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}) is required. This can be installed using the `INSTALL` SQL command. This only needs to be run once.
...
DuckDB can also handle [Google Cloud Storage (GCS)]({% link docs/current/guides/network_cloud_storage/gcs_import.md %}) and [Cloudflare R2]({% link docs/current/guides/network_cloud_storage/cloudflare_r2_import.md %}) via the S3 API.
