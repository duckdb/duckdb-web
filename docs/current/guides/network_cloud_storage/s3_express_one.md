...
DuckDB can read S3 Express One buckets using the [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}).

## Credentials and Configuration

The configuration of S3 Express One buckets is similar to [regular S3 buckets]({% link docs/current/guides/network_cloud_storage/s3_import.md %}) with one exception:
...
For example, to allow DuckDB to use an S3 Express One bucket, configure the [Secrets manager]({% link docs/current/sql/statements/create_secret.md %}) as follows:
...
The “loading only” variant is running the load as part of an [`EXPLAIN ANALYZE`]({% link docs/current/guides/meta/explain_analyze.md %}) statement to measure the runtime without actually creating a local table, while the “creating local table” variant uses [`CREATE TABLE ... AS SELECT`]({% link docs/current/sql/statements/create_table.md %}#create-table--as-select-ctas) to create a persistent table on the local disk.
