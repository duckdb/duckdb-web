...
Loading an Iceberg file from S3 requires both the [`httpfs`]({% link docs/current/core_extensions/httpfs/overview.md %}) and [`iceberg`]({% link docs/current/core_extensions/iceberg/overview.md %}) extensions. Install them using the `INSTALL` SQL command. You only need to install extensions once.
