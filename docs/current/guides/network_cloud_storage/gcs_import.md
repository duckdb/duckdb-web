...
The Google Cloud Storage (GCS) can be used via the [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}).
...
You can [attach to a database file]({% link docs/current/guides/network_cloud_storage/duckdb_over_https_or_s3.md %}) in read-only mode:
