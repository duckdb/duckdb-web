...
To create a new table using the result from a query, use [`CREATE TABLE ... AS SELECT` statement]({% link docs/current/sql/statements/create_table.md %}#create-table--as-select-ctas):
...
We can use DuckDB's [optional `FROM`-first syntax]({% link docs/current/sql/query_syntax/from.md %}) to omit `SELECT *`:
...
For additional options, see the [CSV import reference]({% link docs/current/data/csv/overview.md %}) and the [`COPY` statement documentation]({% link docs/current/sql/statements/copy.md %}).
