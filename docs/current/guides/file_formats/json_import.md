...
To read data from a JSON file, use the `read_json` function in the `FROM` clause of a query:
...
For additional options, see the [JSON Loading reference]({% link docs/current/data/json/overview.md %}) and the [`COPY` statement documentation]({% link docs/current/sql/statements/copy.md %}).
