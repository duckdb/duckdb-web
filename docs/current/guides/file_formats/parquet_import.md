...
To create a new table using the result from a query, use the [`CREATE TABLE ... AS SELECT` statement]({% link docs/current/sql/statements/create_table.md %}#create-table--as-select-ctas):
...
For additional options, see the [Parquet loading reference]({% link docs/current/data/parquet/overview.md %}).
