...
layout: docu
redirect_from:
- /docs/preview/guides/performance/working_with_huge_databases
- /docs/stable/guides/performance/working_with_huge_databases
...
DuckDB's [native database format]({% link docs/current/internals/storage.md %}) supports huge database files without any practical restrictions, however, there are a few things to keep in mind when working with huge database files.
...
2. Checkpointing a DuckDB database can be slow. For example, checkpointing after adding a few rows to a table in the [TPC-H]({% link docs/current/core_extensions/tpch.md %}) SF1000 database takes approximately 5 seconds.
