...
DuckDB automatically creates [zonemaps](https://en.wikipedia.org/wiki/Block_Range_Index) (also known as min-max indexes) for the columns of all [general-purpose data types]({% link docs/current/sql/data_types/overview.md %}#general-purpose-data-types).
...
For an example, let’s repeat the [microbenchmark for timestamps]({% link docs/current/guides/performance/schema.md %}#microbenchmark-using-timestamps) with an ordered timestamp column using an ascending order vs. an unordered one.
...
First, such an index is created implicitly for columns with `PRIMARY KEY`, `FOREIGN KEY`, and `UNIQUE` [constraints]({% link docs/current/guides/performance/schema.md %}#constraints).
Second, explicitly running the [`CREATE INDEX`]({% link docs/current/sql/indexes.md %}) statement creates an ART index on the target column(s).
...
When in doubt, use [`EXPLAIN ANALYZE`]({% link docs/current/guides/meta/explain_analyze.md %}) to verify that your query plan uses the index scan.
...
> * If you define an ART index, do so after bulk loading the data to the table. Adding an index prior to loading, either explicitly or via primary/foreign keys, is [detrimental to load performance]({% link docs/current/guides/performance/schema.md %}#microbenchmark-the-effect-of-primary-keys).
