...
DuckDB can directly connect to [many popular data sources]({% link docs/current/data/data_sources.md %}) and offers several data ingestion methods that allow you to easily and efficiently fill up the database.
...
For a more detailed description, see the [page on the `INSERT` statement]({% link docs/current/data/insert.md %}).

## File Loading: Relative Paths

Use the configuration option [`file_search_path`]({% link docs/current/configuration/overview.md %}#local-configuration-options) to configure to which “root directories” relative paths are expanded on.
...
Alternatively, use the [`read_csv` function]({% link docs/current/data/csv/overview.md %}) to pass along options:
...
DuckDB can create a table from the loaded data using the [`CREATE TABLE ... AS SELECT` statement]({% link docs/current/sql/statements/create_table.md %}#create-table--as-select-ctas):
...
For more details, see the [page on CSV loading]({% link docs/current/data/csv/overview.md %}).
...
Alternatively, use the [`read_parquet` function]({% link docs/current/data/parquet/overview.md %}):
...
For more details, see the [page on Parquet loading]({% link docs/current/data/parquet/overview.md %}).
...
For more details, see the [page on JSON loading]({% link docs/current/data/json/overview.md %}).
