...
All [`LIST` functions]({% link docs/current/sql/functions/list.md %}) work with the [`ARRAY` data type]({% link docs/current/sql/data_types/array.md %}). Additionally, several `ARRAY`-native functions are also supported.
