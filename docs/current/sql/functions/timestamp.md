...
layout: docu
redirect_from:
- /docs/preview/sql/functions/timestamp
- /docs/sql/functions/timestamp
- /docs/stable/sql/functions/timestamp
title: Timestamp Functions
---

<!-- markdownlint-disable MD001 -->

This section describes functions and operators for examining and manipulating [`TIMESTAMP` values]({% link docs/current/sql/data_types/timestamp.md %}).
See also the related [`TIMESTAMPTZ` functions]({% link docs/current/sql/functions/timestamptz.md %}).
...
Adding to or subtracting from [infinite values]({% link docs/current/sql/data_types/timestamp.md %}#special-values) produces the same infinite value.
...
| [`date_diff(part, starttimestamp, endtimestamp)`](#date_diffpart-starttimestamp-endtimestamp) | The number of [`part`]({% link docs/current/sql/functions/datepart.md %}) boundaries between `starttimestamp` and `endtimestamp`, inclusive of the larger timestamp and exclusive of the smaller timestamp. |
| [`date_part([part, ...], timestamp)`](#date_partpart--timestamp) | Get the listed [subfields]({% link docs/current/sql/functions/datepart.md %}) as a `struct`. The list must be constant. |
| [`date_part(part, timestamp)`](#date_partpart-timestamp) | Get [subfield]({% link docs/current/sql/functions/datepart.md %}) (equivalent to `extract`). |
| [`date_sub(part, starttimestamp, endtimestamp)`](#date_subpart-starttimestamp-endtimestamp) | The signed length of the interval between `starttimestamp` and `endtimestamp`, truncated to whole multiples of [`part`]({% link docs/current/sql/functions/datepart.md %}). |
| [`date_trunc(part, timestamp)`](#date_truncpart-timestamp) | Truncate to specified [precision]({% link docs/current/sql/functions/datepart.md %}). |
...
| [`extract(field FROM timestamp)`](#extractfield-from-timestamp) | Get [subfield]({% link docs/current/sql/functions/datepart.md %}) from a timestamp. |
...
| [`strftime(timestamp, format)`](#strftimetimestamp-format) | Converts timestamp to string according to the [format string]({% link docs/current/sql/functions/dateformat.md %}#format-specifiers). |
| [`strptime(text, format-list)`](#strptimetext-format-list) | Converts the string `text` to timestamp applying the [format strings]({% link docs/current/sql/functions/dateformat.md %}) in the list until one succeeds. Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format-list). |
| [`strptime(text, format)`](#strptimetext-format) | Converts the string `text` to timestamp according to the [format string]({% link docs/current/sql/functions/dateformat.md %}#format-specifiers). Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format). |
...
| [`try_strptime(text, format-list)`](#try_strptimetext-format-list) | Converts the string `text` to timestamp applying the [format strings]({% link docs/current/sql/functions/dateformat.md %}) in the list until one succeeds. Returns `NULL` on failure. |
| [`try_strptime(text, format)`](#try_strptimetext-format) | Converts the string `text` to timestamp according to the [format string]({% link docs/current/sql/functions/dateformat.md %}#format-specifiers). Returns `NULL` on failure. |

There are also dedicated extraction functions to get the [subfields]({% link docs/current/sql/functions/datepart.md %}).
...
| **Description** | The signed number of [`part`]({% link docs/current/sql/functions/datepart.md %}) boundaries between `starttimestamp` and `endtimestamp`, inclusive of the larger timestamp and exclusive of the smaller timestamp. |
...
| **Description** | Get the listed [subfields]({% link docs/current/sql/functions/datepart.md %}) as a `struct`. The list must be constant. |
...
| **Description** | Get [subfield]({% link docs/current/sql/functions/datepart.md %}) (equivalent to `extract`). |
...
| **Description** | The signed length of the interval between `starttimestamp` and `endtimestamp`, truncated to whole multiples of [`part`]({% link docs/current/sql/functions/datepart.md %}). |
...
| **Description** | Get [subfield]({% link docs/current/sql/functions/datepart.md %}) from a timestamp. |
...
| **Description** | Converts timestamp to string according to the [format string]({% link docs/current/sql/functions/dateformat.md %}#format-specifiers). |
...
| **Description** | Converts the string `text` to timestamp applying the [format strings]({% link docs/current/sql/functions/dateformat.md %}) in the list until one succeeds. Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format-list). |
...
| **Description** | Converts the string `text` to timestamp according to the [format string]({% link docs/current/sql/functions/dateformat.md %}#format-specifiers). Throws an error on failure. To return `NULL` on failure, use [`try_strptime`](#try_strptimetext-format). |
...
| **Description** | Converts the string `text` to timestamp applying the [format strings]({% link docs/current/sql/functions/dateformat.md %}) in the list until one succeeds. Returns `NULL` on failure. |
...
| **Description** | Converts the string `text` to timestamp according to the [format string]({% link docs/current/sql/functions/dateformat.md %}#format-specifiers). Returns `NULL` on failure. |
