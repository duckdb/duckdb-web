...
| [`equi_width_bins(min, max, bincount, nice := false)`](#equi_width_binsmin-max-bincount-nice--false) | Returns the upper boundaries of a partition of the interval `[min, max]` into `bin_count` equal-sized subintervals (for use with, e.g., [`histogram`]({% link docs/current/sql/functions/aggregates.md %}#histogramargboundaries)). If `nice = true`, then `min`, `max` and `bincount` may be adjusted to produce more aesthetically pleasing results. |
...
| [`is_histogram_other_bin(arg)`](#is_histogram_other_binarg) | Returns `true` when `arg` is the "catch-all element" of its datatype for the purpose of the [`histogram_exact`]({% link docs/current/sql/functions/aggregates.md %}#histogram_exactargelements) function, which is equal to the "right-most boundary" of its datatype for the purpose of the [`histogram`]({% link docs/current/sql/functions/aggregates.md %}#histogramargboundaries) function. |
...
| [`query_table(`*`tbl_names`*`, [`*`by_name`*`])`](#query_tabletbl_names-by_name) | Table function that returns the union of tables given in *`tbl_names`*. If the optional *`by_name`* parameter is set to `true`, it uses [`UNION ALL BY NAME`]({% link docs/current/sql/query_syntax/setops.md %}#union-all-by-name) semantics. |
| [`read_blob(source)`](#read_blobsource) | Returns the content from `source` (a filename, a list of filenames, or a glob pattern) as a `BLOB`. See the [`read_blob` guide]({% link docs/current/guides/file_formats/read_file.md %}#read_blob) for more details. |
...
| [`replace_type(value, source_type, target_type)`](#replace_typevalue-source_type-target_type) | Recursively casts every field of `value` that has type `source_type` to `target_type`. The types are supplied as `NULL`-typed values, e.g., `NULL::VARCHAR`. |
...
| **Description** | Returns the upper boundaries of a partition of the interval `[min, max]` into `bin_count` equal-sized subintervals (for use with, e.g., [`histogram`]({% link docs/current/sql/functions/aggregates.md %}#histogramargboundaries)). If `nice = true`, then `min`, `max` and `bincount` may be adjusted to produce more aesthetically pleasing results.  |
...
| **Description** | Returns `true` when `arg` is the "catch-all element" of its datatype for the purpose of the [`histogram_exact`]({% link docs/current/sql/functions/aggregates.md %}#histogram_exactargelements) function, which is equal to the "right-most boundary" of its datatype for the purpose of the [`histogram`]({% link docs/current/sql/functions/aggregates.md %}#histogramargboundaries) function. |
...
| **Description** | Table function that returns the union of tables given in `tbl_names`. If the optional `by_name` parameter is set to `true`, it uses [`UNION ALL BY NAME`]({% link docs/current/sql/query_syntax/setops.md %}#union-all-by-name) semantics. |
...
| **Description** | Returns the content from `source` (a filename, a list of filenames, or a glob pattern) as a `BLOB`. See the [`read_blob` guide]({% link docs/current/guides/file_formats/read_file.md %}#read_blob) for more details. |
...
| **Description** | Recursively casts every field of `value` that has type `source_type` to `target_type`. The source and target types are supplied as `NULL`-typed values, e.g., `NULL::VARCHAR`. Useful for converting the field types of a deeply nested value (e.g., a `STRUCT` or `LIST`) in one call. |
| **Example** | `replace_type([{'a': '42'}], NULL::VARCHAR, NULL::INTEGER)` |
| **Result** | `[{'a': 42}]` |
...
| **Description** | Return filenames found at the location indicated by the *search_path* in a single column named `file`. The *search_path* may contain [glob pattern matching syntax]({% link docs/current/sql/functions/pattern_matching.md %}). |
| **Example** | `glob('*')` |
