...
redirect_from:
- /docs/preview/sql/statements/create_sequence
...
Sequences can be used as `DEFAULT` values in [`CREATE TABLE` statements]({% link docs/current/sql/statements/create_table.md %}). 

The example below uses a sequence to create an integer [primary key]({% link docs/current/sql/constraints.md %}#primary-key-and-unique-constraint):
...
Sequences can also be added using the [`ALTER TABLE` statement]({% link docs/current/sql/statements/alter_table.md %}). The following example adds an `id` column and fills it with values generated by the sequence:
