...
| `character_maximum_length` | Always `NULL`. DuckDB [text types]({% link docs/current/sql/data_types/text.md %}) do not enforce a value length restriction based on a length type parameter. | `INTEGER` |
...
Apart from the database associated at startup, the list also includes databases that were [attached]({% link docs/current/sql/statements/attach.md %}) later on to the DuckDB process.
...
The `duckdb_prepared_statements()` function provides metadata about the [prepared statements]({% link docs/current/sql/query_syntax/prepared_statements.md %}) that exist in the current DuckDB session.
...
The [`information_schema.schemata`]({% link docs/current/sql/meta/information_schema.md %}#schemata-database-catalog-and-schema) system view provides a more standardized way to obtain metadata about database schemas.
...
Attributes like `temporary`, `start_value` etc. correspond to the various options available in the [`CREATE SEQUENCE`]({% link docs/current/sql/statements/create_sequence.md %}) statement and are documented there in full. Note that the attributes will always be filled out in the `duckdb_sequences` resultset, even if they were not explicitly specified in the `CREATE SEQUENCE` statement.
...
The various settings are described in the [configuration page]({% link docs/current/configuration/overview.md %}).
