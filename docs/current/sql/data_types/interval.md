...
See the [Date Part Functions page]({% link docs/current/sql/functions/datepart.md %}) for a list of available date parts for use with an `INTERVAL`.

See the [Interval Operators page]({% link docs/current/sql/functions/interval.md %}) for functions that operate on intervals.
