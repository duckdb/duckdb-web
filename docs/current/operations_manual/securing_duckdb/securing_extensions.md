...
To lock this configuration, use the [`lock_configuration` option]({% link docs/current/operations_manual/securing_duckdb/overview.md %}#locking-configurations):
...
The [`allow_unsigned_extensions` setting]({% link docs/current/extensions/overview.md %}#unsigned-extensions) can be enabled on start-up to allow loading unsigned extensions.
