...
- /docs/preview/configuration/secrets_manager
- /docs/stable/configuration/secrets_manager
...
Secrets can be created using the [`CREATE SECRET` SQL statement]({% link docs/current/sql/statements/create_secret.md %}).
...
Secrets can be deleted using the [`DROP SECRET` statement]({% link docs/current/sql/statements/create_secret.md %}#syntax-for-drop-secret), e.g.:
...
Secrets can be listed using the built-in table-producing function, e.g., by using the [`duckdb_secrets()` table function]({% link docs/current/sql/meta/duckdb_table_functions.md %}#duckdb_secrets):
