...
`PRAGMA` statements that assign a value to an option can also be issued using the [`SET` statement]({% link docs/current/sql/statements/set.md %}) and the value of an option can be retrieved using `SELECT current_setting(option_name)`.

For DuckDB's built in configuration options, see the [Configuration Reference]({% link docs/current/configuration/overview.md %}#configuration-reference).
DuckDB [extensions]({% link docs/current/extensions/overview.md %}) may register additional configuration options.
...
List all tables, with extra information, similarly to [`DESCRIBE`]({% link docs/current/guides/meta/describe.md %}):
...
See [Storage]({% link docs/current/internals/storage.md %}) for more information.

#### Show Databases

The following statement is equivalent to the [`SHOW DATABASES` statement]({% link docs/current/sql/statements/attach.md %}):
...
> However, certain in-memory data structures such as [vectors]({% link docs/current/internals/vector.md %}) and query results are allocated outside of the buffer manager.
> Additionally, [aggregate functions]({% link docs/current/sql/functions/aggregates.md %}) with complex state (e.g., `list`, `mode`, `quantile`, `string_agg`, and `approx` functions) use memory outside of the buffer manager.
...
Prior to version 1.1.0, DuckDB's [replacement scan mechanism]({% link docs/current/clients/c/replacement_scans.md %}) in Python scanned the global Python namespace. To revert to this old behavior, use the following setting:
...
The format of this identifier matches the platform name as described in the [extension loading explainer]({% link docs/current/extensions/extension_distribution.md %}#platforms):
...
The profiling documentation contains an overview of the available [metrics]({% link docs/current/dev/profiling.md %}#metrics).
...
The available optimizations can be queried using the [`duckdb_optimizers()` table function]({% link docs/current/sql/meta/duckdb_table_functions.md %}#duckdb_optimizers).
...
The `create_fts_index` and `drop_fts_index` options are only available when the [`fts` extension]({% link docs/current/core_extensions/full_text_search.md %}) is loaded. Their usage is documented on the [Full-Text Search extension page]({% link docs/current/core_extensions/full_text_search.md %}).
...
When [`CHECKPOINT`]({% link docs/current/sql/statements/checkpoint.md %}) is called when no changes are made, force a checkpoint regardless:
...
It is also possible to set the block size on a per-file basis, see [`ATTACH`]({% link docs/current/sql/statements/attach.md %}) for details.
