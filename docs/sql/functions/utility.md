...
| `query_table(`*`tbl_name`*`, [`*`by_name`*`])` | Returns the table given in *`tbl_name`*. If the optional *`by_name`* parameter is set to `true`, then ??. Warning: this function allows invoking arbitrary queries, potentially altering the database state. | `query(t1)` | (the rows of `t1`) |
