...
The corresponding `TIMESTAMP WITHOUT TIME ZONE` stores the same single `INT64`, but arithmetic, binning, and string formatting follow the straightforward rules of UTC without offsets or time zones. Accordingly, `TIMESTAMP`s could be interpreted as UTC timestamps, but more commonly they are used to represent *local* observations of time recorded in an unspecified time zone, and operations on these types can be interpreted as simply manipulating tuple fields following nominal temporal logic.
