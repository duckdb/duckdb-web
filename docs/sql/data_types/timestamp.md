...
The corresponding `TIMESTAMP WITHOUT TIME ZONE` stores the same single `INT64` number, but arithmetic, binning, and string formatting follow the straightforward rules of UTC without offsets or time zones. Accordingly, `TIMESTAMP`s could be interpreted as UTC timestamps, but more commonly they are used to represent *local* values of time recorded by an observer in an unspecified time zone and operations on these types can be interpreted as mechanical manipulation of record fields.
