...
Therefore, it is not recommended to use `row` for constructing the result of a query.
Use [`struct_pack`](#creating-structs) instead:
...
#### `row` Function with a Column Name, a Constant, and an Expression as Input
