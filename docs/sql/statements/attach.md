...
> Warning Closing the connection, e.g., invoking the [`close()` function in Python]({% link docs/clients/python/dbapi.md %}#connection), does not release the locks held on the database files as the file handles are held by the main DuckDB instance (in Python's case, the `duckdb` module).
