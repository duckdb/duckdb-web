...
| `auto_type_candidates` | This option allows you to specify the types that the sniffer will use when detecting CSV column types. (e.g., `SELECT * FROM read_csv_auto('csv_file.csv', auto_type_candidates=['BIGINT', 'VARCHAR'])` | `TYPE[]` | `['VARCHAR', 'TIMESTAMP', 'DATE', 'TIME', 'DOUBLE', 'BIGINT', 'BOOLEAN', 'SQLNULL']` |
