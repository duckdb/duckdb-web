...
We can use the [`FROM`-first syntax]({% link docs/preview/sql/query_syntax/from.md %}#from-first-syntax) to omit `SELECT *`.
...
| `auto_detect` | [Auto detect CSV parameters]({% link docs/preview/data/csv/auto_detection.md %}). | `BOOL` | `true` |
...
| `dateformat` | [Date format]({% link docs/preview/sql/functions/dateformat.md %}) used when parsing and writing dates. | `VARCHAR` | (empty) |
...
| `hive_partitioning` | Interpret the path as a [Hive partitioned path]({% link docs/preview/data/partitioning/hive_partitioning.md %}). | `BOOL` | `false` |
...
| `names` or `column_names` | Column names, as a list. See [example]({% link docs/preview/data/csv/tips.md %}#provide-names-if-the-file-does-not-contain-a-header). | `VARCHAR[]` | (empty) |
...
| `rejects_table` | Name of the [temporary table where information on faulty lines is stored]({% link docs/preview/data/csv/reading_faulty_csv_files.md %}#reject-errors). | `VARCHAR` | `reject_errors` |
...
| `sample_size` | Number of sample lines for [auto detection of parameters]({% link docs/preview/data/csv/auto_detection.md %}). | `BIGINT` | 20480 |
...
The `auto_type_candidates` option lets you specify the data types that should be considered by the CSV reader for [column data type detection]({% link docs/preview/data/csv/auto_detection.md %}#type-detection).
Usage example:
...
DuckDB supports reading erroneous CSV files. For details, see the [Reading Faulty CSV Files page]({% link docs/preview/data/csv/reading_faulty_csv_files.md %}).

## Order Preservation

The CSV reader respects the `preserve_insertion_order` [configuration option]({% link docs/preview/configuration/overview.md %}) to [preserve insertion order]({% link docs/preview/sql/dialect/order_preservation.md %}).
...
DuckDB can write CSV files using the [`COPY ... TO` statement]({% link docs/preview/sql/statements/copy.md %}#copy--to).
