...
* Table macros return a tabular result: when called, they act essentially as [table functions]({% link docs/preview/sql/query_syntax/from.md %}#table-functions) and return a table value. Their expression can be a `SELECT` statement, or a call to another table function.
...
* Optionally, a parameter may explicitly specify a particular datatype. This can be any of the existing DuckDB [data types]({% link docs/preview/sql/data_types/overview.md %}). Note: there is no way to specify a parameter of a `TABLE` type.
...
This is similar to calling a [function]({% link docs/preview/sql/functions/overview.md %}).
...
* If the expression of a scalar macro references an [aggregate function]({% link docs/preview/sql/functions/aggregates.md %}), then the macro behaves like an aggregate function too.
* A call to a table macro can appear in the `FROM` clause of a `SELECT` statement, or in a [`CALL` statement]({% link docs/preview/sql/statements/call.md %}).
