...
* Making SQL clauses more compact and readable:
    ...
    * [Specifying a percentage of the table size for the `LIMIT` clause]({% link docs/preview/sql/query_syntax/limit.md %}): write `LIMIT 10%` to return 10% of the query results.
* Transforming tables:
...
* [`SWITCH` statements as syntactic sugar for the `CASE` expression]({% link docs/preview/sql/expressions/case.md %}#switch-expression)
...
* [Underscores as digit separators in numeric literals]({% link docs/preview/sql/data_types/literal_types.md %}#underscores-in-numeric-literals)
