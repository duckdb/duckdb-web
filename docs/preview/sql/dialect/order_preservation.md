...
* `UNION`
* `USING SAMPLE`
* Whole-table aggregation (the input order, that is, the order in which rows are fed into [order-sensitive aggregate functions]({% link docs/preview/sql/functions/aggregates.md %}#order-by-clause-in-aggregate-functions) is not guaranteed unless explicitly specified in the aggregate function)
...
* [CSV reader]({% link docs/preview/data/csv/overview.md %}#order-preservation) (`read_csv` function)
* [JSON reader]({% link docs/preview/data/json/overview.md %}#order-preservation) (`read_json` function)
* [Parquet reader]({% link docs/preview/data/parquet/overview.md %}#order-preservation) (`read_parquet` function)

Preservation of insertion order is controlled by the `preserve_insertion_order` [configuration option]({% link docs/preview/configuration/overview.md %}).
