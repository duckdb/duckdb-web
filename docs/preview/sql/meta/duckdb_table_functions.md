...
This function lists both built-in CRSs as well as the ones that are installed by extensions (such as the [`SPATIAL`]({% link docs/preview/core_extensions/spatial/overview.md %}) extension. 
...
| `keyword_category` | The category of the keyword: `reserved` (cannot be used as an identifier without quoting), `unreserved` (can be used as an identifier), `column_name` (usable as a column name but not as a type or function name), or `type_function` (usable as a type or function name but not as a column name). See [Keywords and Identifiers]({% link docs/preview/sql/dialect/keywords_and_identifiers.md %}). | `VARCHAR` |
