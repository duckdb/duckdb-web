...
Unnesting is an operation that decomposes values of a [composite types]({% link docs/preview/sql/data_types/overview.md %}#nested--composite-types) into its components.
Values of the [`LIST`]({% link docs/preview/sql/data_types/list.md %}) and [`STRUCT`]({% link docs/preview/sql/data_types/struct.md %}) type may be unnested using the [`unnest()` function]({% link docs/preview/sql/functions/list.md %}#unnestlist-recursive-max_depth).
...
The `unnest` function can be called in the [`SELECT`-clause]({% link docs/preview/sql/query_syntax/select.md %}) as if it is a scalar function.
...
Unnesting a value of the `LIST`-type yields only the element values. To also keep track of their indices (the subscripts), you can use the built-in macro [`generate_subscripts()`]({% link docs/preview/sql/functions/list.md %}#generate_subscriptslist-dimension).
The `generate_subscripts` macro takes a value of the `LIST`-type as first argument.

### `unnest()` as Table Function

Since a call to `unnest` on a value of the `LIST`-type yields a rowset, it may also be treated as a [table function]({% link docs/preview/sql/query_syntax/from.md %}#table-functions). 
This means it may appear in the [`FROM`-clause]({% link docs/preview/sql/query_syntax/from.md %}) or a [`CALL`-statement]({% link docs/preview/sql/statements/call.md %}).
