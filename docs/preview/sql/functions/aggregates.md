...
Return two values, the total sum of `amount` and the sum of `amount` minus columns where the region is `north` using the [`FILTER` clause]({% link docs/preview/sql/query_syntax/filter.md %}):
...
To exclude `NULL`s from `list`, you can use a [`FILTER` clause]({% link docs/preview/sql/query_syntax/filter.md %}).
