...
Lambdas are passed as argument to certain [list functions]({% link docs/preview/sql/functions/list.md %}), which evaluate the passed lambda expression against its elements. 
...
Lambdas may also be used in the [`COLUMNS` clause]({% link docs/preview/sql/expressions/star.md %}#columns-lambda-function).
