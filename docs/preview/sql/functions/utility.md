...
| **Description** | Returns the value of the SQL variable named `variable_name`, or `NULL` if it is not set. See the [`SET VARIABLE` statement]({% link docs/preview/sql/statements/set_variable.md %}) for more details. |
...
A [table function]({% link docs/preview/sql/query_syntax/from.md %}#table-functions) is used in place of a table in a `FROM` clause.
