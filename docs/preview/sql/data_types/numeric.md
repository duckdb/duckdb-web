...
But sometimes you need numbers that are even bigger than what is supported by a `HUGEINT`! In these situations, you can use the `VARINT` type, which stores positive numbers in a similar fashion as other integer types, but uses three additional bytes to store the required size and a sign bit. Therefore, a number with `N` decimal digits requires approximately `0.415 * N + 3` bytes when stored in a `VARINT`. 

Unlike `VARINT` implementations in other systems, there are limits: The maximal and minimal representable values are approximately `±4.27e20201778`. Those are numbers with 20,201,779 decimal digits and storing a single number of this size requires 8 megabytes. 
