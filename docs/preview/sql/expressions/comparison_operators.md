...
> It is not possible to enforce stricter type-checking for DuckDB's comparison operators. If you require stricter type-checking, we recommend creating a [macro]({% link docs/preview/sql/statements/create_macro.md %}) with the [`typeof` function]({% link docs/preview/sql/functions/utility.md %}#typeofexpression) or implementing a [user-defined function]({% link docs/preview/clients/python/function.md %}).
