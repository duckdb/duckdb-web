...
See the [`STRUCT` data type]({% link docs/preview/sql/data_types/struct.md %}) and [`STRUCT` functions]({% link docs/preview/sql/functions/struct.md %}) pages for more details on working with structs.
...
The [pattern matching operators]({% link docs/preview/sql/functions/pattern_matching.md %}) `LIKE`, `GLOB`, `SIMILAR TO` and their variants allow you to select columns by matching their names to patterns.
...
`COLUMNS` expressions can be used in [`DISTINCT ON`]({% link docs/preview/sql/query_syntax/select.md %}#distinct-on-clause) clauses to specify distinct columns by pattern:
...
`COLUMNS` also supports passing in a [lambda function]({% link docs/preview/sql/functions/lambda.md %}). 
