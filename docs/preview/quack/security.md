...
See [Securing Quack with a Reverse Proxy]({% link docs/preview/quack/setup/reverse_proxy.md %}) for [nginx](https://nginx.org/) and [Caddy recipes](https://caddyserver.com/) (production and local-test).
...
The cleanest way to plug in custom authentication is through using a [`MACRO`]({% link docs/preview/sql/statements/create_macro.md %}).
