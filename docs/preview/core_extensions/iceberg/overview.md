...
* [Writing to Iceberg]({% link docs/preview/core_extensions/iceberg/writing_to_iceberg.md %}) describes the supported write operations, partitioning and schema evolution.
* [Iceberg Functions]({% link docs/preview/core_extensions/iceberg/iceberg_functions.md %}) documents the functions provided by the extension.
* [Iceberg Options]({% link docs/preview/core_extensions/iceberg/iceberg_options.md %}) documents the scan parameters, `ATTACH` options, secret options and settings.
* [Catalogs]({% link docs/preview/core_extensions/iceberg/catalogs.md %}) explains how to attach an Iceberg REST Catalog, with instructions for specific catalogs.
* [Troubleshooting]({% link docs/preview/core_extensions/iceberg/troubleshooting.md %}) lists common problems and their solutions.
...
Use the [`iceberg_scan` function]({% link docs/preview/core_extensions/iceberg/iceberg_functions.md %}#iceberg_scan) to read an Iceberg table from a path:
...
For the full list of parameters accepted by `iceberg_scan`, see the [Iceberg Options]({% link docs/preview/core_extensions/iceberg/iceberg_options.md %}#scan-options) page.

### Reading from Object Stores

The `iceberg` extension works together with the [`httpfs` extension]({% link docs/preview/core_extensions/httpfs/overview.md %}) or the [`azure` extension]({% link docs/preview/core_extensions/azure.md %}) to access Iceberg tables in object stores such as S3 or Azure Blob Storage.
...
For full read and write access — and to use Iceberg's catalog features — attach an Iceberg REST catalog. Most catalogs authenticate with OAuth2; store the credentials in a [secret]({% link docs/preview/configuration/secrets_manager.md %}) and attach the catalog with `ATTACH ... (TYPE iceberg, ...)`:
...
The [metadata functions]({% link docs/preview/core_extensions/iceberg/reference.md %}#read-and-metadata-functions) also work on catalog tables when passed a fully qualified name, e.g., `iceberg_snapshots(my_catalog.default.events)`.

* For the full set of write operations — partitioning, `UPDATE`, `DELETE`, `MERGE INTO`, `ALTER TABLE`, and table properties — see [Writing to Iceberg]({% link docs/preview/core_extensions/iceberg/writing.md %}).
* For catalog-specific setup and the complete list of `ATTACH` options and secret parameters, see [Iceberg REST Catalogs]({% link docs/preview/core_extensions/iceberg/iceberg_rest_catalogs.md %}).
...
The `iceberg_to_ducklake` function performs a metadata-only copy of an attached Iceberg catalog into a [DuckLake]({% link docs/preview/core_extensions/ducklake.md %}) catalog, letting you query Iceberg tables as if they were DuckLake tables:
