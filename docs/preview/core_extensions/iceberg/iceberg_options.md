...
This page lists the options of the `iceberg` extension: the parameters accepted by the [Iceberg functions]({% link docs/preview/core_extensions/iceberg/iceberg_functions.md %}), the options of the `ATTACH` and `CREATE SECRET` statements used to [connect to a catalog]({% link docs/preview/core_extensions/iceberg/catalogs.md %}), and the global settings.
