...
The `iceberg` extension supports writing to Iceberg tables that are managed by an [Iceberg REST Catalog]({% link docs/preview/core_extensions/iceberg/iceberg_rest_catalogs.md %}). All write operations go through the attached catalog and are committed as new Iceberg snapshots.

> Writing requires an attached catalog. The path-based `iceberg_scan` interface described in the [overview]({% link docs/preview/core_extensions/iceberg/overview.md %}) is read-only. To write, first [attach an Iceberg REST catalog]({% link docs/preview/core_extensions/iceberg/iceberg_rest_catalogs.md %}).
...
See the [Functions and Settings Reference]({% link docs/preview/core_extensions/iceberg/reference.md %}#table-and-schema-property-functions) for the equivalent schema (namespace) property functions.
...
`MERGE INTO` performs an upsert against a source relation. The join key is given with a second `USING` clause (see the [`MERGE INTO` statement]({% link docs/preview/sql/statements/merge_into.md %})):
...
Because the full DDL and DML set is supported, [`COPY FROM DATABASE`]({% link docs/preview/sql/statements/copy.md %}#copy-from-database--to) can perform deep copies between Iceberg and DuckDB storage in either direction:
...
To copy an Iceberg catalog into [DuckLake]({% link docs/preview/core_extensions/ducklake.md %}), see [Interoperability with DuckLake]({% link docs/preview/core_extensions/iceberg/overview.md %}#interoperability-with-ducklake).
