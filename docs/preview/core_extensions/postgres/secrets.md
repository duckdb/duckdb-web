...
User credentials and other PostgreSQL database connection details can be stored using the DuckDB [Secrets Manager]({% link docs/preview/configuration/secrets_manager.md %}). The following syntax can be used to create a secret:
...
The secret of type `rds` requires the `aws` extension to be installed and allows to configure AWS Credential Chain the same way as with the secret of type `s3`, see details in the [AWS extension documentation]({% link docs/preview/core_extensions/aws.md %}#credential_chain-provider).

## Storing Secrets inside a PostgreSQL Database

DuckDB [Secrets Manager]({% link docs/preview/configuration/secrets_manager.md %}) supports pluggable Storage Providers.
