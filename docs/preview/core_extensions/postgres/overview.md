...
The `postgres` extension will be transparently [autoloaded]({% link docs/preview/extensions/overview.md %}#autoloading-extensions) on first use from the official extension repository.
...
PostgreSQL connection information can also be specified with [secrets]({% link docs/preview/configuration/secrets_manager.md %}), see details on the [corresponding page]({% link docs/preview/core_extensions/postgres/secrets.md %}).
...
You can use the [postgres_hstore_get]({% link docs/preview/core_extensions/postgres/functions.md %}#postgres_hstore_get)
function to read the value for a given key, or [postgres_hstore_to_json]({% link docs/preview/core_extensions/postgres/functions.md %}#postgres_hstore_to_json)
