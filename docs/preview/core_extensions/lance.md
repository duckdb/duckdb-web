...
To access object store URIs (e.g., `s3://...`), configure a `TYPE lance` secret using the [Secrets Manager]({% link docs/preview/sql/statements/create_secret.md %}):
...
Use the [`COPY ... TO ...` statement]({% link docs/preview/sql/statements/copy.md %}#copy--to) to materialize query results as a Lance dataset.
...
To write to `s3://...` paths, configure a `TYPE lance` secret for that scope using the [Secrets Manager]({% link docs/preview/sql/statements/create_secret.md %}):
