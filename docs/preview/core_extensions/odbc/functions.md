...
> Warning Using `odbc_copy` from [Python Relational API]({% link docs/preview/clients/python/relational_api.md %}).
>
> `odbc_copy` is a table function that [returns](#returns-5) one row for each 2048 copied rows.
> When it is used from Python with `duckdb.sql()` the [Lazy Evaluation]({% link docs/preview/clients/python/relational_api.md %}#lazy-evaluation)
> is taking place. Thus no rows will be copied until [a method that triggers execution]({% link docs/preview/clients/python/relational_api.md %}#output)
