...
Parquet files written by some legacy writers do not set the `UTF8` flag on string columns, so DuckDB reads them as `BLOB`. `ResultSet.getObject()` then returns a `DuckDBBlobResult` and `ResultSet.getString()` returns the bytes rendered as an escaped string rather than the expected text. Enable the [`binary_as_string`]({% link docs/preview/data/parquet/overview.md %}) setting to read these columns as `VARCHAR`:
...
In a [GraalVM Native Image]({% link docs/preview/clients/java/deploy_native_image.md %}) executable, opening the first connection may fail with:
...
The shared library was neither embedded in the executable nor found next to it. Either add a resource entry for your platform's library or place the library file beside the executable. Both options are described on the [Deploy as Native Image]({% link docs/preview/clients/java/deploy_native_image.md %}) page.
...
* [Deploy as Native Image]({% link docs/preview/clients/java/deploy_native_image.md %}) — building standalone executables with GraalVM, including both ways to provide the shared library.
* [Java (JDBC) Client]({% link docs/preview/clients/java/overview.md %}) — installing the driver from Maven Central, the fix for the driver-not-found errors above.
* [Define Connections]({% link docs/preview/clients/java/connecting.md %}) — driver registration, configuration options, and instance behavior behind many connection-time errors.
* [Parquet Files]({% link docs/preview/data/parquet/overview.md %}) — the `binary_as_string` setting and other options for reading Parquet string columns correctly.
