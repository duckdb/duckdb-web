...
Whether two `getConnection` calls share a `dbAddress` follows the same rules as [instance caching]({% link docs/preview/clients/java/connecting.md %}#database-instances-and-instance-caching):
...
* [Define Connections]({% link docs/preview/clients/java/connecting.md %}) — the `jdbc_jfr_memory_monitor` property and the query-cancel scheduler thread the driver runs.
* [Profiling]({% link docs/preview/dev/profiling.md %}) — DuckDB's query profiling output and the formats `getProfilingInformation()` returns.
* [Run Queries]({% link docs/preview/clients/java/querying.md %}) — running the queries whose progress, timing, and profiles this page observes.
