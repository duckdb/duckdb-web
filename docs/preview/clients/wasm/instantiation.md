...
This page shows the patterns for a jsDelivr CDN, webpack, Vite and statically served files. Once instantiated, the `db` object is used to [import data]({% link docs/preview/clients/wasm/data_ingestion.md %}) and [run queries]({% link docs/preview/clients/wasm/query.md %}).
...
* `coi` — adds threading for parallel query execution; it requires the page to be [cross-origin isolated]({% link docs/preview/clients/wasm/deploying_duckdb_wasm.md %}#cross-origin-isolation) and must be opted into explicitly (see [Threading](#threading))
...
* `allowUnsignedExtensions`: allow loading extensions that are not signed (see [Load Extensions]({% link docs/preview/clients/wasm/extensions.md %})).
...
* The page must be served with the `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp` headers, which is what makes the browser expose `SharedArrayBuffer` and report the page as cross-origin isolated. See [Cross-Origin Isolation]({% link docs/preview/clients/wasm/deploying_duckdb_wasm.md %}#cross-origin-isolation).
...
* [Import Data]({% link docs/preview/clients/wasm/data_ingestion.md %}) — registering files and inserting data into the instantiated database.
* [Run Queries]({% link docs/preview/clients/wasm/query.md %}) — executing queries against the `db` object created here.
* [Deploy]({% link docs/preview/clients/wasm/deploying_duckdb_wasm.md %}) — serving the library, worker, and WebAssembly components that these bundles reference.
* [DuckDB Wasm Client]({% link docs/preview/clients/wasm/overview.md %}) — the layered API and the examples the snippets above are drawn from.
