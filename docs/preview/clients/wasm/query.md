...
* [Import Data]({% link docs/preview/clients/wasm/data_ingestion.md %}) — loading the data that these queries read.
* [Load Extensions]({% link docs/preview/clients/wasm/extensions.md %}) — the Parquet extension used by the export example and other query-time extensions.
* [Prepared Statements]({% link docs/preview/sql/query_syntax/prepared_statements.md %}) — DuckDB's SQL-level support for the parameterized queries shown here.
* [Instantiate]({% link docs/preview/clients/wasm/instantiation.md %}) — creating the `db` that connections are opened on.
* [Troubleshoot]({% link docs/preview/clients/wasm/troubleshoot.md %}) — streaming results with `send()` to stay within the memory limit.
