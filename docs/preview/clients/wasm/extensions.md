...
A growing subset of extensions is supported for DuckDB-Wasm, spanning [core extensions]({% link docs/preview/core_extensions/overview.md %}), [community extensions]({% link docs/preview/extensions/community_extensions.md %}), and external extensions. Most are fetched on the fly when they are autoloaded or explicitly loaded, rather than being bundled into the DuckDB-Wasm binary. For example:
...
| [spatial]({% link docs/preview/core_extensions/spatial/overview.md %})  | Adds support for geospatial data types and functions             |                 |
...
The [MDN website](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) is a great resource for more information regarding CORS. If a query over a remote file fails with a network error, see [Troubleshoot]({% link docs/preview/clients/wasm/troubleshoot.md %}#network-error-when-querying-remote-files).
...
Note that `GET` requests for the extensions must be [CORS enabled](https://www.w3.org/wiki/CORS_Enabled) for a browser to allow the connection; see [Troubleshoot]({% link docs/preview/clients/wasm/troubleshoot.md %}#extension-fails-to-load-from-a-custom-repository) if a load fails.
...
* [Deploy]({% link docs/preview/clients/wasm/deploying_duckdb_wasm.md %}) — serving and mirroring extensions from a custom endpoint.
* [Core Extensions]({% link docs/preview/core_extensions/overview.md %}) — the core extensions available across all DuckDB clients.
* [Community Extensions]({% link docs/preview/extensions/community_extensions.md %}) — third-party extensions installable with `INSTALL … FROM community`.
* [Import Data]({% link docs/preview/clients/wasm/data_ingestion.md %}) — using the Parquet, JSON, and Wasm-flavored httpfs extensions to read files.
* [Troubleshoot]({% link docs/preview/clients/wasm/troubleshoot.md %}) — CORS errors when loading extensions from a custom repository.
