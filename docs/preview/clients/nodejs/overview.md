...
The API for this client is somewhat compliant with the SQLite Node.js client for easier transition.
...
All options as described on [Database configuration]({% link docs/preview/configuration/overview.md %}#configuration-reference) can be (optionally) supplied to the `Database` constructor as second argument. The third argument can be optionally supplied to get feedback on the given options.
...
[Apache Arrow]({% link docs/preview/guides/python/sql_on_arrow.md %}) can be used to insert data into DuckDB without making a copy:
...
To load [unsigned extensions]({% link docs/preview/extensions/overview.md %}#unsigned-extensions), instantiate the database as follows:
