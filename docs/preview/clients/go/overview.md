...
This example follows the client's [`simple` example](https://github.com/duckdb/duckdb-go/tree/main/examples/simple). [Run Queries]({% link docs/preview/clients/go/querying.md %}) covers sending queries, binding parameters, and reading results in full.

### Data Source Names

The DSN passed to `sql.Open()` is the database path followed by optional [DuckDB configuration options]({% link docs/preview/configuration/overview.md %}) as URL-style query parameters:
...
To run initialization steps, such as `SET` statements, before the first query, open the database with `sql.OpenDB()` and a `Connector`. See [Connect]({% link docs/preview/clients/go/connecting.md %}) for connectors, configuration, and the connection lifetime.
...
| Build tag | Enables |
|--|--|
| `duckdb_arrow` | The [Apache Arrow interface]({% link docs/preview/clients/go/result_handling.md %}). It is a heavy dependency, so it is opt-in. |
| `duckdb_use_lib` | Dynamically link against a DuckDB library on the system instead of statically linking the bundled one. See [Troubleshoot]({% link docs/preview/clients/go/troubleshoot.md %}#linking-a-dynamic-library). |
| `duckdb_use_static_lib` | Statically link against a custom DuckDB static library instead of the bundled one. See [Troubleshoot]({% link docs/preview/clients/go/troubleshoot.md %}#linking-a-custom-static-library). |

## Bundled Extensions

Every pre-built library statically links DuckDB's default extensions: [ICU]({% link docs/preview/core_extensions/icu.md %}), [JSON]({% link docs/preview/data/json/overview.md %}), [Parquet]({% link docs/preview/data/parquet/overview.md %}), and [Autocomplete]({% link docs/preview/core_extensions/autocomplete.md %}). [Automatic extension loading]({% link docs/preview/extensions/overview.md %}#autoloading-extensions) is also enabled, so other core extensions install and load on first use.
...
Moving to v2 also introduced a few breaking changes, including opt-in Arrow support and stricter JSON scanning. See [Troubleshoot]({% link docs/preview/clients/go/troubleshoot.md %}#scanning-json-values) and the [README](https://github.com/duckdb/duckdb-go#breaking-changes) for the complete list.

## Further Reading

* [Connect]({% link docs/preview/clients/go/connecting.md %}) — opening in-memory and file-backed databases, DSN configuration, connectors, and the connection lifetime.
* [Run Queries]({% link docs/preview/clients/go/querying.md %}) — `Exec`, `Query`, prepared statements, parameter binding, transactions, and scanning results into Go values.
* [Import Data]({% link docs/preview/clients/go/data_import.md %}) — bulk loading with the Appender and reading directly from Parquet, CSV, and JSON files.
* [Handle Results]({% link docs/preview/clients/go/result_handling.md %}) — the Apache Arrow interface for columnar result exchange.
* [Write User Defined Functions]({% link docs/preview/clients/go/functions.md %}) — scalar and table user-defined functions, and replacement scans.
* [Profile and Monitor]({% link docs/preview/clients/go/profiling.md %}) — query profiling and logging.
* [Troubleshoot]({% link docs/preview/clients/go/troubleshoot.md %}) — linking, cgo, Windows setup, and other build and runtime issues.
* [Clients Overview]({% link docs/preview/clients/overview.md %}) — the other client APIs DuckDB provides alongside Go.
