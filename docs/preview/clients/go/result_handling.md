...
DuckDB is columnar, and beyond reading a result set [row by row]({% link docs/preview/clients/go/querying.md %}#scanning-results) through `database/sql`, the Go client can exchange whole results with [Apache Arrow](https://arrow.apache.org/) as a stream of record batches, and register an Arrow stream as a queryable view. Both are described below.

## Enabling the Arrow Interface

The DuckDB Arrow interface is a heavy dependency, so starting with v2 it is opt-in. Enable it with the `duckdb_arrow` [build tag]({% link docs/preview/clients/go/overview.md %}#build-tags):
...
`QueryContext()` accepts bound parameters as trailing arguments, the same as [`database/sql` queries]({% link docs/preview/clients/go/querying.md %}#binding-parameters).
...
* [Run Queries]({% link docs/preview/clients/go/querying.md %}) — sending the queries whose results this page reads, and reading them row by row.
* [Build Tags]({% link docs/preview/clients/go/overview.md %}#build-tags) — the `duckdb_arrow` tag that enables the Arrow interface, and the other build tags.
* [Write User Defined Functions]({% link docs/preview/clients/go/functions.md %}) — replacement scans, another way to expose external data to SQL.
