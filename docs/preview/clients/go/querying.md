...
The `Context` variants take a [`context.Context`](https://pkg.go.dev/context) as their first argument, which is the recommended form because it lets a query be cancelled or time out. The sections below cover sending statements, binding parameters, prepared statements, transactions, and scanning results. For opening the database these run on, see [Connect]({% link docs/preview/clients/go/connecting.md %}).

## Sending Statements

Use `Exec` (or `ExecContext`) for a statement that does not return rows, such as [`INSERT`]({% link docs/preview/sql/statements/insert.md %}) or [`UPDATE`]({% link docs/preview/sql/statements/update.md %}). It returns a `sql.Result`, from which `RowsAffected()` reports how many rows changed:
...
> Warning Do *not* use prepared statements to insert large amounts of data into DuckDB. See [Import Data]({% link docs/preview/clients/go/data_import.md %}) for the Appender, which is far faster for bulk inserts.

### Forcing a Parameter Type

The client infers the DuckDB type of a bound value from its Go type. When that inference is not what you want, wrap the value with `duckdb.Typed()` to pin the DuckDB [logical type]({% link docs/preview/sql/data_types/overview.md %}). This matters most for timestamps: a bare `time.Time` binds as `TIMESTAMP_TZ`, so binding against a `TIMESTAMP_NS` column needs the hint:
...
`duckdb.Typed()` is a scalar binding hint; DuckDB validates the value against the requested type when the parameter is bound. See [Troubleshoot]({% link docs/preview/clients/go/troubleshoot.md %}#timestamp-versus-timestamp_tz) for the details of how the client maps `time.Time` to DuckDB's timestamp types.
...
DuckDB's [nested types]({% link docs/preview/sql/data_types/overview.md %}#nested--composite-types), such as `LIST`, `STRUCT`, and `MAP`, and its [`JSON`]({% link docs/preview/data/json/overview.md %}) type, are scanned through the generic `duckdb.Composite[T]` wrapper. Choose the Go type parameter to match the SQL shape, then read the decoded value with `Get()`:
...
This follows the client's [`json` example](https://github.com/duckdb/duckdb-go/tree/main/examples/json). To read whole result sets as columnar batches instead of row by row, see [Handle Results]({% link docs/preview/clients/go/result_handling.md %}).

## Further Reading

* [Handle Results]({% link docs/preview/clients/go/result_handling.md %}) — reading results as Apache Arrow record batches instead of row by row.
* [Import Data]({% link docs/preview/clients/go/data_import.md %}) — the Appender, the recommended alternative to prepared statements for bulk inserts.
...
* [Connect]({% link docs/preview/clients/go/connecting.md %}) — opening the database that these statements run on.
