...
DuckDB can read many file formats directly in SQL, which is often the simplest way to load external data: the Go client just sends the query. Because the [Parquet]({% link docs/preview/data/parquet/overview.md %}), [CSV]({% link docs/preview/data/csv/overview.md %}), and [JSON]({% link docs/preview/data/json/overview.md %}) readers are among the [bundled extensions]({% link docs/preview/clients/go/overview.md %}#bundled-extensions), no extra installation is needed. Query a file in place with the matching table function:

```go
rows, err := db.QueryContext(ctx, `SELECT * FROM read_parquet('data.parquet')`)
```

The same pattern reads CSV with `read_csv()` and JSON with `read_json()`. To load a file into a table rather than query it in place, wrap the reader in a [`CREATE TABLE ... AS SELECT`]({% link docs/preview/sql/statements/create_table.md %}#create-table--as-select-ctas) or [`COPY`]({% link docs/preview/sql/statements/copy.md %}) statement sent with `Exec`:
...
* [Appender]({% link docs/preview/data/appender.md %}) — the engine-level Appender interface that the Go `Appender` wraps.
* [Data Import]({% link docs/preview/data/overview.md %}) — DuckDB's full set of bulk-loading options.
* [Run Queries]({% link docs/preview/clients/go/querying.md %}) — sending the `CREATE TABLE` and `COPY` statements the file readers build on.
* [Connect]({% link docs/preview/clients/go/connecting.md %}) — creating the `Connector` and connection the Appender operates on.
