...
There are several components of a query that are highlighted in different colors. The colors can be configured using the `.highlight_colors` [dot command]({% link docs/preview/clients/cli/dot_commands.md %}):
...
In addition, 8-bit colors can be used. Run `.display_colors` to list all available color names, see [8-Bit Colors]({% link docs/preview/clients/cli/friendly_cli.md %}#8-bit-colors).
