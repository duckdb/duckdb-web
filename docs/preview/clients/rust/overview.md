...
This example is adapted from the crate's [`basic` example](https://github.com/duckdb/duckdb-rs/blob/main/crates/duckdb/examples/basic.rs). [Run Queries]({% link docs/preview/clients/rust/querying.md %}) covers sending queries and reading results in full.
...
| Feature | Enables |
|--|--|
| `json` | Reading and writing [JSON]({% link docs/preview/data/json/overview.md %}), statically linked. |
| `parquet` | Reading and writing [Parquet]({% link docs/preview/data/parquet/overview.md %}), statically linked. |
...
| `icu` | The [ICU extension]({% link docs/preview/core_extensions/icu.md %}) for locale-aware and time-zone-aware operations, which the plain `bundled` build omits. |
...
| Feature | Enables |
|--|--|
| `vtab` | Base support for [table functions]({% link docs/preview/clients/rust/functions.md %}#table-functions). |
...
| `vscalar` | [Scalar functions]({% link docs/preview/clients/rust/functions.md %}#scalar-functions). Implies `vtab-arrow`. |
| `vscalar-arrow` | Arrow-optimized scalar functions, intended for use alongside `vscalar`. |
| `appender-arrow` | Append Arrow `RecordBatch`es through the [Appender]({% link docs/preview/clients/rust/data_import.md %}#appender). Implies `vtab-arrow`. |
| `loadable-extension` | Experimental. Build a loadable DuckDB extension rather than a client application. See [Building a Loadable Extension]({% link docs/preview/clients/rust/functions.md %}#building-a-loadable-extension). |
...
| Feature | Enables |
|--|--|
| `polars` | Exchange query results as [Polars](https://pola.rs/) data frames. See [Handle Results]({% link docs/preview/clients/rust/result_handling.md %}#polars-data-frames). |
| `r2d2` | A connection pool through the [`r2d2`](https://crates.io/crates/r2d2) crate. See [Connect]({% link docs/preview/clients/rust/connecting.md %}#connection-pooling). |
...
* [Connect]({% link docs/preview/clients/rust/connecting.md %}) — opening in-memory and file-backed databases, `Config` options, connection pooling, and thread safety.
* [Run Queries]({% link docs/preview/clients/rust/querying.md %}) — `execute()`, prepared statements, parameter binding, and mapping rows to Rust types.
* [Import Data]({% link docs/preview/clients/rust/data_import.md %}) — bulk loading with the Appender and reading directly from Parquet, CSV, and JSON files.
* [Handle Results]({% link docs/preview/clients/rust/result_handling.md %}) — Apache Arrow and Polars result interchange.
* [Write User Defined Functions]({% link docs/preview/clients/rust/functions.md %}) — user-defined scalar and table functions, and loadable extensions.
* [Profile and Monitor]({% link docs/preview/clients/rust/profiling.md %}) — query profiling and interrupting long-running queries.
* [Troubleshoot]({% link docs/preview/clients/rust/troubleshoot.md %}) — linking, the ICU extension, and extension-versus-client build issues.
* [Clients Overview]({% link docs/preview/clients/overview.md %}) — the other client APIs DuckDB provides alongside Rust.
