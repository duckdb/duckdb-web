...
* [Run Queries]({% link docs/preview/clients/rust/querying.md %}) — sending the queries whose results this page reads, and reading them row by row.
* [Write User Defined Functions]({% link docs/preview/clients/rust/functions.md %}) — writing table functions, of which the built-in `ArrowVTab` is one.
* [Import Data]({% link docs/preview/clients/rust/data_import.md %}) — appending Arrow record batches into a table with the `appender-arrow` feature.
