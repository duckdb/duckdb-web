...
Prepared statements work with both. The sections below walk through sending statements, binding parameters, and turning rows into typed Rust values. For opening the `Connection` these run on, see [Connect]({% link docs/preview/clients/rust/connecting.md %}).

## Sending Statements

Use `execute()` for a single statement that does not return rows, such as [`INSERT`]({% link docs/preview/sql/statements/insert.md %}) or [`UPDATE`]({% link docs/preview/sql/statements/update.md %}). It takes the SQL and a list of parameters, and returns the number of affected rows. Pass an empty slice `[]` when there are no parameters:
...
> Warning Do *not* use prepared statements to insert large amounts of data into DuckDB. See [Import Data]({% link docs/preview/clients/rust/data_import.md %}) for the Appender and other faster options.
...
`Value`, `ValueRef`, and `Type` are marked non-exhaustive, so a `match` over them should include a wildcard arm to stay forward-compatible as DuckDB adds types. To read whole result sets as columnar batches instead of row by row, see [Handle Results]({% link docs/preview/clients/rust/result_handling.md %}).

## Further Reading

* [Handle Results]({% link docs/preview/clients/rust/result_handling.md %}) — reading results as Apache Arrow record batches or Polars data frames instead of row by row.
* [Import Data]({% link docs/preview/clients/rust/data_import.md %}) — the Appender, the recommended alternative to prepared statements for bulk inserts.
...
* [Connect]({% link docs/preview/clients/rust/connecting.md %}) — opening the `Connection` that these statements run on.
