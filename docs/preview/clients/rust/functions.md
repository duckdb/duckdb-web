...
* [Handle Results]({% link docs/preview/clients/rust/result_handling.md %}) — the built-in `ArrowVTab` table function and the Arrow vector API shared with user-defined functions.
* [C API]({% link docs/preview/clients/c/api.md %}) — the table function interface that the Rust `VTab` API mirrors.
* [Extensions]({% link docs/preview/core_extensions/overview.md %}) — DuckDB's extension system that a loadable Rust extension plugs into.
* [Run Queries]({% link docs/preview/clients/rust/querying.md %}) — calling registered functions from SQL.
