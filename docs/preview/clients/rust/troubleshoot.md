...
Because crates.io imposes a 10 MB package-size limit, the `bundled` source excludes the [ICU extension]({% link docs/preview/core_extensions/icu.md %}). As a result, some collation and time-zone-aware date/time operations fail out of the box. Install and load ICU at runtime to restore them:
...
Do not enable `loadable-extension` for a client application. Use it only when building an extension. See [Write User Defined Functions]({% link docs/preview/clients/rust/functions.md %}#building-a-loadable-extension) for how loadable extensions are built and packaged.
...
* [Rust Client]({% link docs/preview/clients/rust/overview.md %}) — installation, the `bundled` feature, and the full list of Cargo feature flags.
* [Connect]({% link docs/preview/clients/rust/connecting.md %}) — opening connections, whose failures are often build or linking problems.
* [Write User Defined Functions]({% link docs/preview/clients/rust/functions.md %}) — the `loadable-extension` feature and how to package an extension correctly.
