...
The full list of settings is on the [Configuration page]({% link docs/preview/configuration/overview.md %}). Many of them can also be changed after connecting with a [`SET` statement]({% link docs/preview/sql/statements/set.md %}) or the equivalent [`PRAGMA`]({% link docs/preview/configuration/pragmas.md %}).
...
DuckDB runs queries on its own pool of native threads, sized by the `threads` option and shared by all connections to a database instance. See the [DuckDB concurrency documentation]({% link docs/preview/connect/concurrency.md %}) for how connections and threads interact.
...
* [Run Queries]({% link docs/preview/clients/rust/querying.md %}) — using the `Connection` to send queries and read results.
* [Configuration]({% link docs/preview/configuration/overview.md %}) — the full list of DuckDB settings that can be passed to `Config`.
* [Concurrency]({% link docs/preview/connect/concurrency.md %}) — how DuckDB handles multiple connections and threads.
* [Troubleshoot]({% link docs/preview/clients/rust/troubleshoot.md %}) — linking and build problems encountered when opening a connection.
