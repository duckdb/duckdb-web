...
To disable `jemalloc`, [build DuckDB from source]({% link docs/preview/dev/building/overview.md %}) and set the `SKIP_EXTENSIONS` flag as follows:
...
The macOS version of DuckDB does with `jemalloc` but can be [built from source]({% link docs/preview/dev/building/macos.md %}) to include it:
