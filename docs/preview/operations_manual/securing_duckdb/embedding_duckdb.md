...
> Tip Calling DuckDB's CLI client via shell commands is **not recommended** for embedding DuckDB. It is recommended to use one of the client libraries, e.g., [Python]({% link docs/preview/clients/python/overview.md %}), [R]({% link docs/preview/clients/r.md %}), [Java]({% link docs/preview/clients/java/overview.md %}), etc.
