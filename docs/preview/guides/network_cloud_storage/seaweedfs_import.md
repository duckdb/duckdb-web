...
SeaweedFS also supports Iceberg: table buckets store the table data as Parquet files, and the gateway's built-in Iceberg REST Catalog serves the table metadata. See the [SeaweedFS catalog example]({% link docs/preview/core_extensions/iceberg/catalogs.md %}#seaweedfs) to query Iceberg tables through it.
