...
To load a CSV file directly over HTTP(S), the [`httpfs` extension]({% link docs/preview/core_extensions/httpfs/overview.md %}) is required. This can be installed using the `INSTALL` SQL command. This only needs to be run once.
...
After the `httpfs` extension is set up, CSV files can be read over `http(s)` the same way as a local file, using [`read_csv`]({% link docs/preview/guides/file_formats/csv_import.md %}) (or `read_csv_auto` for older DuckDB versions):

```sql
SELECT * FROM read_csv('https://⟨domain⟩/path/to/file.csv');
```

Moreover, the `read_csv` function itself can also be omitted thanks to DuckDB's [replacement scan mechanism]({% link docs/preview/clients/c/replacement_scans.md %}):
