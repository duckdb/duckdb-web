...
To fetch credentials automatically instead — or to use a named profile, SSO, or an assumed role — see the [`aws` extension]({% link docs/preview/core_extensions/aws.md %}#credential_chain-provider). The default looks like:
...
DuckDB can also handle [Google Cloud Storage (GCS)]({% link docs/preview/guides/network_cloud_storage/gcs_import.md %}) and [Cloudflare R2]({% link docs/preview/guides/network_cloud_storage/cloudflare_r2_import.md %}) via the S3 API.
