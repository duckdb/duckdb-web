...
First, create a secret of type `rds`. It uses the [AWS credential chain]({% link docs/preview/core_extensions/aws.md %}#credential_chain-provider) to mint the IAM token:
...
* [Postgres extension secrets — AWS RDS IAM Authentication]({% link docs/preview/core_extensions/postgres/secrets.md %}#aws-rds-iam-authentication) — the full reference, including verifying the setup with `psql` first.
* [AWS extension]({% link docs/preview/core_extensions/aws.md %}#credential_chain-provider) — the credential chain options available to the `rds` secret.
