...
Indexes can significantly increase memory consumption. Additionally, they are [currently not buffer-managed]({% link docs/preview/guides/performance/indexing.md %}#indexes-and-memory). Avoid using indexes if possible. If your workload requires indexing large tables, consider reducing the memory limit to accommodate this.
