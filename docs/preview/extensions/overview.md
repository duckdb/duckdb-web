...
For example, to use the built-in [`json` extension]({% link docs/preview/data/json/overview.md %}) to read a JSON file:
...
> Extension installation and loading are subject to a few [limitations]({% link docs/preview/extensions/installing_extensions.md %}#limitations).
...
Community extensions are contributed by community members but they are built, [signed]({% link docs/preview/extensions/extension_distribution.md %}#signed-extensions), and distributed in a centralized repository.
...
See the [installation instructions]({% link docs/preview/extensions/installing_extensions.md %}) and the [advanced installation methods page]({% link docs/preview/extensions/advanced_installation_methods.md %}).
