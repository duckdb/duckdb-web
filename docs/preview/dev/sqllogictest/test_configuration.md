...
Command line options `--settings`, `--on-init` and `--init-script` can be used to specify [DuckDB configuration]({% link docs/preview/configuration/overview.md %}).
...
| `--settings <list[struct]>` | Set [DuckDB configuration]({% link docs/preview/configuration/overview.md %}), struct should have keys name and value |
