...
# Visualising DuckDB databases in Tableau
...
For example, this will create a view of the `h2oai` parquet test file in the current DuckDB database:
...
1. [Download](https://github.com/duckdb/duckdb/releases/download/v0.7.1/duckdb_jdbc-osx-universal.jar)the DuckDB JDBC driver. This is a file called `duckdb_jdbc-osx-universal.jar`. Make sure this is from the same build as the version of DuckDB used to create the database file!
2. Follow the directions and copy this file to either `/Library/JDBC` (for access by all users) or `~/Library/JDBC` (for access by your login alone) (e.g., `cp ~/Downloads/duckdb_jdbc-osx-universal.jar ~/Library/JDBC`)
