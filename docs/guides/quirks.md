...
- Case insensitivity and the resulting inability to `SELECT A FROM 'file.parquet'` when `a` is in the file too and first. That's actually a DuckDB thing. Great when not working with external data, who wants to need to remember the correct capitalization and otherwise get the wrong numbers?
