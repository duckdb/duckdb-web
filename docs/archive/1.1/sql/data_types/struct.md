...
Structs can be created using the [`struct_pack(name := expr, ...)`]({% link docs/archive/1.1/sql/functions/struct.md %}) function, the equivalent array notation `{'name': expr, ...}`, using a row variable, or using the `row` function.
...
Retrieving a value from a struct can be accomplished using dot notation, bracket notation, or through [struct functions]({% link docs/archive/1.1/sql/functions/struct.md %}) like `struct_extract`.
...
The `STRUCT` type can be compared using all the [comparison operators]({% link docs/archive/1.1/sql/expressions/comparison_operators.md %}).
...
See [Struct Functions]({% link docs/archive/1.1/sql/functions/struct.md %}).