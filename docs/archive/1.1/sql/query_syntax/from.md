...
Semi joins provide the same logic as the [`IN` operator]({% link docs/archive/1.1/sql/expressions/in.md %}) statement.
...
relational tables are not ordered, but imported tables such as [data frames]({% link docs/archive/1.1/api/python/data_ingestion.md %}#pandas-dataframes-–-object-columns)
or disk files (like [CSVs]({% link docs/archive/1.1/data/csv/overview.md %}) or [Parquet files]({% link docs/archive/1.1/data/parquet/overview.md %})) do have a natural ordering.
