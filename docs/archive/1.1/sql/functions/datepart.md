...
The `date_part` and `date_diff` and `date_trunc` functions can be used to manipulate the fields of temporal types such as [`DATE`]({% link docs/archive/1.1/sql/data_types/date.md %}) and [`TIMESTAMP`]({% link docs/archive/1.1/sql/data_types/timestamp.md %}).
