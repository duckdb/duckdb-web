...
layout: docu
redirect_from:
- /docs/archive/1.1/sql/case_sensitivity
- /docs/archive/1.1/sql/case_sensitivity/
- /docs/archive/1.1/sql/keywords-and-identifiers
- /docs/archive/1.1/sql/dialect/keywords-and-identifiers
...
* Unquoted identifiers need to conform to a number of rules:
    * They must not be a reserved keyword (see [`duckdb_keywords()`]({% link docs/archive/1.1/sql/meta/duckdb_table_functions.md %}#duckdb_keywords)), e.g., `SELECT 123 AS SELECT` will fail.
...
Additionally, it is best practice to avoid DuckDB's two internal [database schema names]({% link docs/archive/1.1/sql/meta/duckdb_table_functions.md %}#duckdb_databases), `system` and `temp`.
...
To change this behavior, set the `preserve_identifier_case` [configuration option]({% link docs/archive/1.1/configuration/overview.md %}#configuration-reference) to `false`.
...
With the `preserve_identifier_case` [configuration option]({% link docs/archive/1.1/configuration/overview.md %}#configuration-reference) set to `false`, all identifiers are turned into lowercase:
