...
We can use DuckDB's [optional `FROM`-first syntax]({% link docs/archive/1.1/sql/query_syntax/from.md %}) to omit `SELECT *`:
...
For additional options, see the [CSV import reference]({% link docs/archive/1.1/data/csv/overview.md %}) and the [`COPY` statement documentation]({% link docs/archive/1.1/sql/statements/copy.md %}).