...
Either connect to a new [in-memory DuckDB]({% link docs/archive/1.1/api/python/dbapi.md %}#in-memory-connection), the [default connection]({% link docs/archive/1.1/api/python/dbapi.md %}#default-connection) or a file backed database:
...
> The `%sql` command and `duckdb.sql` share the same [default connection]({% link docs/archive/1.1/api/python/dbapi.md %}) if you provide `duckdb:///:default:` as the SQLAlchemy connection string.
...
DuckDB's [httpfs extension]({% link docs/archive/1.1/extensions/httpfs/overview.md %}) allows Parquet and CSV files to be queried remotely over http.
...
DuckDB can be used to process local [Parquet files]({% link docs/archive/1.1/data/parquet/overview.md %}) as well, which may be desirable if querying the entire Parquet file, or running multiple queries that require large subsets of the file.
