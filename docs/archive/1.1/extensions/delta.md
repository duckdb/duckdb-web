...
The `delta` extension will be transparently [autoloaded]({% link docs/archive/1.1/extensions/overview.md %}#autoloading-extensions) on first use from the official extension repository.
...
To scan a Delta table in an [S3 bucket]({% link docs/archive/1.1/extensions/httpfs/s3api.md %}), run:
...
For authenticating to S3 buckets, DuckDB [Secrets]({% link docs/archive/1.1/configuration/secrets_manager.md %}) are supported:
...
To scan a Delta table in an [Azure Blob Storage bucket]({% link docs/archive/1.1/extensions/azure.md %}#azure-blob-storage), run:
...
For authenticating to Azure Blob Storage, DuckDB [Secrets]({% link docs/archive/1.1/configuration/secrets_manager.md %}) are supported:
...
Support for the [other DuckDB platforms]({% link docs/archive/1.1/extensions/working_with_extensions.md %}#platforms) is work-in-progress.