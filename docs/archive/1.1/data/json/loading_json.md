...
| `hive_partitioning` | Whether or not to interpret the path as a [Hive partitioned path]({% link docs/archive/1.1/data/partitioning/hive_partitioning.md %}). | `BOOL` | `false` |
...
| `dateformat` | Specifies the date format to use when parsing dates. See [Date Format]({% link docs/archive/1.1/sql/functions/dateformat.md %}) | `VARCHAR` | `'iso'` |
...
| `timestampformat` | Specifies the date format to use when parsing timestamps. See [Date Format]({% link docs/archive/1.1/sql/functions/dateformat.md %}) | `VARCHAR` | `'iso'`|
| `union_by_name` | Whether the schema's of multiple JSON files should be [unified]({% link docs/archive/1.1/data/multiple_files/combining_schemas.md %}) | `BOOL` | `false` |
...
When the `json` extension is installed, `FORMAT JSON` is supported for `COPY FROM`, `IMPORT DATABASE`, as well as `COPY TO`, and `EXPORT DATABASE`. See the [`COPY` statement]({% link docs/archive/1.1/sql/statements/copy.md %}) and the [`IMPORT` / `EXPORT` clauses]({% link docs/archive/1.1/sql/statements/export.md %}).
