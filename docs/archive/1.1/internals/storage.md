...
Check [`EXPORT` documentation]({% link docs/archive/1.1/sql/statements/export.md %}) for more details on the syntax.
...
Several features in DuckDB, including [parallelism]({% link docs/archive/1.1/guides/performance/how_to_tune_workloads.md %}) and [compression]({% post_url 2022-10-28-lightweight-compression %}) are based on row groups.
...
2. Open the database with the latest version of DuckDB, export it to a standard format (e.g., Parquet), then import it using to any version of DuckDB. See the [`EXPORT/IMPORT DATABASE` statements]({% link docs/archive/1.1/sql/statements/export.md %}) for details.