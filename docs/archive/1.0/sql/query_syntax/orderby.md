...
`ORDER BY` is an output modifier. Logically it is applied near the very end of the query (just prior to [`LIMIT`]({% link docs/archive/1.0/sql/query_syntax/limit.md %}) or [`OFFSET`]({% link docs/archive/1.0/sql/query_syntax/limit.md %}), if present).
...
For more information on collations, see the [Collation page]({% link docs/archive/1.0/sql/expressions/collations.md %}).
