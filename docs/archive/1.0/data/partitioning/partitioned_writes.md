...
When the `partition_by` clause is specified for the [`COPY` statement]({% link docs/archive/1.0/sql/statements/copy.md %}), the files are written in a [Hive partitioned]({% link docs/archive/1.0/data/partitioning/hive_partitioning.md %}) folder hierarchy. The target is the name of the root directory (in the example above: `orders`). The files are written in-order in the file hierarchy. Currently, one file is written per thread to each directory.
