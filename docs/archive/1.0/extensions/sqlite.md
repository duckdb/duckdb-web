...
layout: docu
redirect_from:
- docs/archive/1.0/extensions/sqlite_scanner
...
The `sqlite` extension will be transparently [autoloaded]({% link docs/archive/1.0/extensions/overview.md %}#autoloading-extensions) on first use from the official extension repository.
...
> Deprecated The old `sqlite_attach` function is deprecated. It is recommended to switch over to the new [`ATTACH` syntax]({% link docs/archive/1.0/sql/statements/attach.md %}).