...
Prior to version 0.10.0, DuckDB did not have a [Secrets manager]({% link docs/archive/1.0/sql/statements/create_secret.md %}). Hence, the configuration of and authentication to S3 endpoints was handled via variables. This page documents the legacy authentication scheme for the S3 API.

> The recommended way to configuration and authentication of S3 endpoints is to use [secrets]({% link docs/archive/1.0/extensions/httpfs/s3api.md %}#configuration-and-authentication).
...
The [`aws` extension]({% link docs/archive/1.0/extensions/aws.md %}) allows for loading AWS credentials.
