...
**Problem:**
The build fails on macOS when both the [`httpfs` extension]({% link docs/archive/1.0/extensions/httpfs/overview.md %}) and the Python package are included:
...
**Solution:**
In general, we recommended using the nightly builds, available under GitHub main on the [installation page]({% link docs/archive/1.0/installation/index.html %}).
...
**Problem:**
When building the [`httpfs` extension]({% link docs/archive/1.0/extensions/httpfs/overview.md %}) on Linux, the build may fail with the following error.
