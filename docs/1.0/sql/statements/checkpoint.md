...
Checkpoint operations happen automatically based on the WAL size (see [Configuration]({% link docs/1.0/configuration/overview.md %})). This
...
Also see the related [`PRAGMA` option]({% link docs/1.0/configuration/pragmas.md %}#force-checkpoint) for further behavior modification.
...
> Warning The [`VACUUM` statement]({% link docs/1.0/sql/statements/vacuum.md %}) does _not_ trigger vacuuming deletes and hence does not reclaim space.