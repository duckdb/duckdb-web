...
> The [`UNPIVOT` statement]({% link docs/1.0/sql/statements/unpivot.md %}) is the inverse of the `PIVOT` statement.
...
The `PIVOT` statement may be included within a `SELECT` statement as a CTE ([a Common Table Expression, or `WITH` clause]({% link docs/1.0/sql/query_syntax/with.md %})), or a subquery.
...
[`ENUM` types]({% link docs/1.0/sql/data_types/enum.md %}) are used to find the distinct values that should become columns.
