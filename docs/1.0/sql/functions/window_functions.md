...
Window functions are [blocking operators]({% link docs/1.0/guides/performance/how_to_tune_workloads.md %}#blocking-operators), i.e., they require their entire input to be buffered, making them one of the most memory-intensive operators in SQL.
...
All [aggregate functions]({% link docs/1.0/sql/functions/aggregates.md %}) can be used in a windowing context, including the optional [`FILTER` clause]({% link docs/1.0/sql/query_syntax/filter.md %}).
...
the canonical order of the [`SELECT statement`]({% link docs/1.0/sql/statements/select.md %}).

### Filtering the Results of Window Functions Using `QUALIFY`

Window functions are executed after the [`WHERE`]({% link docs/1.0/sql/query_syntax/where.md %}) and [`HAVING`]({% link docs/1.0/sql/query_syntax/having.md %}) clauses have been already evaluated, so it's not possible to use these clauses to filter the results of window functions
The [`QUALIFY` clause]({% link docs/1.0/sql/query_syntax/qualify.md %}) avoids the need for a subquery or [`WITH` clause]({% link docs/1.0/sql/query_syntax/with.md %}) to perform this filtering.
