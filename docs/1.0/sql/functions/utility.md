...
layout: docu
redirect_from:
- docs/archive/1.0/test/functions/utility
...
| [`getenv(var)`](#getenvvar) | Returns the value of the environment variable `var`. Only available in the [command line client]({% link docs/1.0/api/cli/overview.md %}). |
...
| [`read_blob(source)`](#read_blobsource) | Returns the content from `source` (a filename, a list of filenames, or a glob pattern) as a `BLOB`. See the [`read_blob` guide]({% link docs/1.0/guides/file_formats/read_file.md %}#read_blob) for more details. |
...
| **Description** | Returns the value of the environment variable `var`. Only available in the [command line client]({% link docs/1.0/api/cli/overview.md %}). |
| **Example** | `getenv('HOME')` |
...
| **Description** | Returns the content from `source` (a filename, a list of filenames, or a glob pattern) as a `BLOB`. See the [`read_blob` guide]({% link docs/1.0/guides/file_formats/read_file.md %}#read_blob) for more details. |
...
| Name | Description |
|:--|:-------|
| [`glob(search_path)`](#globsearch_path) | Return filenames found at the location indicated by the *search_path* in a single column named `file`. The *search_path* may contain [glob pattern matching syntax]({% link docs/1.0/sql/functions/pattern_matching.md %}). |
...
| **Description** | Return filenames found at the location indicated by the *search_path* in a single column named `file`. The *search_path* may contain [glob pattern matching syntax]({% link docs/1.0/sql/functions/pattern_matching.md %}). |
| **Example** | `glob('*')` |
