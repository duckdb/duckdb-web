...
Structs can be created using the [`struct_pack(name := expr, ...)`]({% link docs/1.0/sql/functions/nested.md %}#struct-functions) function, the equivalent array notation `{'name': expr, ...}`, using a row variable, or using the `row` function.
...
Retrieving a value from a struct can be accomplished using dot notation, bracket notation, or through [struct functions]({% link docs/1.0/sql/functions/nested.md %}#struct-functions) like `struct_extract`.
...
The `STRUCT` type can be compared using all the [comparison operators]({% link docs/1.0/sql/expressions/comparison_operators.md %}).
