...
A timestamp specifies a combination of [`DATE`]({% link docs/1.0/sql/data_types/date.md %}) (year, month, day) and a [`TIME`]({% link docs/1.0/sql/data_types/time.md %}) (hour, minute, second, microsecond). Timestamps can be created using the `TIMESTAMP` keyword, where the data must be formatted according to the ISO 8601 format (`YYYY-MM-DD hh:mm:ss[.zzzzzz][+-TT[:tt]]`). Decimal places beyond the targeted sub-second precision are ignored.
...
See [Timestamp Functions]({% link docs/1.0/sql/functions/timestamp.md %}).

## Time Zones

The `TIMESTAMPTZ` type can be binned into calendar and clock bins using a suitable extension.
The built-in [ICU extension]({% link docs/1.0/extensions/icu.md %}) implements all the binning and arithmetic functions using the
...
You can also find a reference table of [available time zones]({% link docs/1.0/sql/data_types/timezones.md %}).

## Calendars

The [ICU extension]({% link docs/1.0/extensions/icu.md %}) also supports non-Gregorian calendars using the `SET Calendar` command.
