...
* Unquoted identifiers need to conform to a number of rules:
    * They must not be a reserved keyword (see [`duckdb_keywords()`]({% link docs/1.0/sql/meta/duckdb_table_functions.md %}#duckdb_keywords)), e.g., `SELECT 123 AS SELECT` will fail.
...
Additionally, it is best practice to avoid DuckDB's two internal [database schema names]({% link docs/1.0/sql/meta/duckdb_table_functions.md %}#duckdb_databases), `system` and `temp`.
...
To change this behavior, set the `preserve_identifier_case` [configuration option]({% link docs/1.0/configuration/overview.md %}#configuration-reference) to `false`.
...
With the `preserve_identifier_case` [configuration option]({% link docs/1.0/configuration/overview.md %}#configuration-reference) set to `false`, all identifiers are turned into lowercase:
