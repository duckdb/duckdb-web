...
The list includes most, but not all, available core extensions. For the full list, we maintain a [list of core extensions]({% link docs/1.0/extensions/core_extensions.md %}).
...
For example, to use the built-in [`json` extension]({% link docs/1.0/extensions/json.md %}) to read a JSON file:
...
To make the DuckDB distribution lightweight, only a few essential extensions are built-in, varying slightly per distribution. Which extension is built-in on which platform is documented in the [list of core extensions]({% link docs/1.0/extensions/core_extensions.md %}#default-extensions).
...
> Extension installation and loading are subject to a few [limitations]({% link docs/1.0/extensions/working_with_extensions.md %}#limitations).
...
to install and load the [`spatial` extension]({% link docs/1.0/extensions/spatial.md %}), run:
...
DuckDB will automatically install and load the [`httpfs`]({% link docs/1.0/extensions/httpfs/overview.md %}) extension. No explicit `INSTALL` or `LOAD` statements are required.
...
To see which extensions can be autoloaded, check the [core extensions list]({% link docs/1.0/extensions/core_extensions.md %}).

### Community Extensions

DuckDB supports installing third-party [Community Extensions]({% link docs/1.0/extensions/community_extensions.md %}).
...
API to install and load extensions. For example the [Python API client]({% link docs/1.0/api/python/overview.md %}#loading-and-installing-extensions),
...
to the [Client API docs]({% link docs/1.0/api/overview.md %})
...
For more details on extension version refer to [Extension Versioning]({% link docs/1.0/extensions/versioning_of_extensions.md %}).
...
See the [Working with Extensions page]({% link docs/1.0/extensions/working_with_extensions.md %}#platforms) for details on available platforms.
...
To load unsigned extensions using the [CLI client]({% link docs/1.0/api/cli/overview.md %}), pass the `-unsigned` flag to it on startup:
...
For Client APIs, the `allow_unsigned_extensions` database configuration options needs to be set, see the respective [Client API docs]({% link docs/1.0/api/overview.md %}).
For example, for the Python client, see the [Loading and Installing Extensions section in the Python API documentation]({% link docs/1.0/api/python/overview.md %}#loading-and-installing-extensions).

## Working with Extensions

For advanced installation instructions and more details on extensions, see the [Working with Extensions page]({% link docs/1.0/extensions/working_with_extensions.md %}).