...
* `wasm_eh` and `wasm_mvp` (see [DuckDB-Wasm's extensions]({% link docs/1.0/api/wasm/extensions.md %}))
...
If the custom repository is served over HTTPS or S3, the [`httpfs` extension]({% link docs/1.0/extensions/httpfs/overview.md %}) is required. DuckDB will attempt to [autoload]({% link docs/1.0/extensions/overview.md %}#autoloading-extensions)
...
For building and installing extensions from source, see the [building guide]({% link docs/1.0/dev/building/build_instructions.md %}#building-and-installing-extensions-from-source).
