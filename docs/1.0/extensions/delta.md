...
The `delta` extension will be transparently [autoloaded]({% link docs/1.0/extensions/overview.md %}#autoloading-extensions) on first use from the official extension repository.
...
For authenticating to S3 buckets, DuckDB [Secrets]({% link docs/1.0/configuration/secrets_manager.md %}) are supported:
...
Support for the [other DuckDB platforms]({% link docs/1.0/extensions/working_with_extensions.md %}#platforms) is work-in-progress.