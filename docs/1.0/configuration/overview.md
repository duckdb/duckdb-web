...
The configuration options can be set using either the [`SET` statement]({% link docs/1.0/sql/statements/set.md %}) or the [`PRAGMA` statement]({% link docs/1.0/configuration/pragmas.md %}).
They can be reset to their original values using the [`RESET` statement]({% link docs/1.0/sql/statements/set.md %}#reset).

The values of configuration options can be queried via the [`current_setting()` scalar function]({% link docs/1.0/sql/functions/utility.md %}) or using the [`duckdb_settings()` table function]({% link docs/1.0/sql/meta/duckdb_table_functions.md %}#duckdb_settings). For example:
...
DuckDB has a [Secrets manager]({% link docs/1.0/sql/statements/create_secret.md %}), which provides a unified user interface for secrets across all backends (e.g., AWS S3) that use them.

## Configuration Reference

<!-- This section is generated by scripts/generate_config_docs.py -->

Configuration options come with different default [scopes]({% link docs/1.0/sql/statements/set.md %}#scopes): `GLOBAL` and `LOCAL`. Below is a list of all available configuration options by scope.
