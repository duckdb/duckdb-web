...
Using a [glob pattern]({% link docs/1.0/data/multiple_files/overview.md %}#glob-syntax) upon read or a [Hive partitioning]({% link docs/1.0/data/partitioning/hive_partitioning.md %}) structure are good ways to transparently handle multiple files.
...
See the [Performance Guide on file formats]({% link docs/1.0/guides/performance/file_formats.md %}#parquet-file-sizes) for more tips.