...
In the [C++ API]({% link docs/1.0/api/cpp.md %}), the Appender works as follows:
...
You can change this by explicitly using [transactions]({% link docs/1.0/sql/statements/transactions.md %}) and surrounding your batches of `AppendRow` calls by `BEGIN TRANSACTION` and `COMMIT` statements.
...
* [C]({% link docs/1.0/api/c/appender.md %})
* [Go]({% link docs/1.0/api/go.md %}#appender)
* [Julia]({% link docs/1.0/api/julia.md %}#appender-api)
* [JDBC (Java)]({% link docs/1.0/api/java.md %}#appender)
* [Rust]({% link docs/1.0/api/rust.md %}#appender)