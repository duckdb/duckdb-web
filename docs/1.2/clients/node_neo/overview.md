...
layout: docu
redirect_from:
- /docs/1.2/api/node_neo/overview
- /docs/1.2/api/node_neo/overview/
- /docs/1.2/clients/node_neo/overview
...
It depends on low-level bindings that adhere closely to [DuckDB's C API]({% link docs/1.2/clients/c/overview.md %}),
...
* Lossless & efficent support for values of all [DuckDB data types]({% link docs/1.2/sql/data_types/overview.md %}).
* Wraps [released DuckDB binaries](https://github.com/duckdb/duckdb/releases) instead of rebuilding DuckDB.
* Built on [DuckDB's C API]({% link docs/1.2/clients/c/overview.md %}); exposes more functionality.
...
Set [configuration options]({% link docs/1.2/configuration/overview.md %}):
```ts
const instance = await DuckDBInstance.create('my_duckdb.db', {
  threads: '4'
});
