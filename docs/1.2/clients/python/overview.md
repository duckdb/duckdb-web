...
- /docs/1.2/api/python
- /docs/1.2/api/python/
- /docs/1.2/api/python/overview
- /docs/1.2/api/python/overview/
- /docs/1.2/clients/python/overview
title: Python API
---
...
DuckDB can ingest data from a wide variety of formats – both on-disk and in-memory. See the [data ingestion page]({% link docs/1.2/clients/python/data_ingestion.md %}) for more information.
...
Note that these are read-only, i.e., editing these tables via [`INSERT`]({% link docs/1.2/sql/statements/insert.md %}) or [`UPDATE` statements]({% link docs/1.2/sql/statements/update.md %}) is not possible.
...
DuckDB supports converting query results efficiently to a variety of formats. See the [result conversion page]({% link docs/1.2/clients/python/conversion.md %}) for more information.
...
DuckDB supports writing Relation objects directly to disk in a variety of formats. The [`COPY` statement]({% link docs/1.2/sql/statements/copy.md %}) can be used to write data to disk using SQL as an alternative.
...
The `duckdb.connect()` accepts a `config` dictionary, where [configuration options]({% link docs/1.2/configuration/overview.md %}#configuration-reference) can be specified. For example:
...
The `DuckDBPyConnection` object is not thread-safe. If you would like to write to the same database from multiple threads, create a cursor for each thread with the [`DuckDBPyConnection.cursor()` method]({% link docs/1.2/clients/python/reference/index.md %}#duckdb.DuckDBPyConnection.cursor).

## Loading and Installing Extensions

DuckDB's Python API provides functions for installing and loading [extensions]({% link docs/1.2/extensions/overview.md %}), which perform the equivalent operations to running the `INSTALL` and `LOAD` SQL commands, respectively. An example that installs and loads the [`spatial` extension]({% link docs/1.2/extensions/spatial/overview.md %}) looks like follows:
...
To load [unsigned extensions]({% link docs/1.2/extensions/overview.md %}#unsigned-extensions), use the `config = {"allow_unsigned_extensions": "true"}` argument with the `duckdb.connect()` method.
