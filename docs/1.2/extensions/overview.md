...
The list includes most, but not all, available core extensions. For the full list, we maintain a [list of core extensions]({% link docs/1.2/extensions/core_extensions.md %}).
...
For example, to use the built-in [`json` extension]({% link docs/1.2/data/json/overview.md %}) to read a JSON file:
...
To make the DuckDB distribution lightweight, only a few essential extensions are built-in, varying slightly per distribution. Which extension is built-in on which platform is documented in the [list of core extensions]({% link docs/1.2/extensions/core_extensions.md %}#default-extensions).
...
> Extension installation and loading are subject to a few [limitations]({% link docs/1.2/extensions/installing_extensions.md %}#limitations).
...
to install and load the [`spatial` extension]({% link docs/1.2/extensions/spatial/overview.md %}), run:
...
DuckDB will automatically install and load the [`httpfs`]({% link docs/1.2/extensions/httpfs/overview.md %}) extension. No explicit `INSTALL` or `LOAD` statements are required.
...
To see which extensions can be autoloaded, check the [core extensions list]({% link docs/1.2/extensions/core_extensions.md %}).
...
Community extensions are contributed by community members but they are built, [signed]({% link docs/1.2/extensions/extension_distribution.md %}#signed-extensions), and distributed in a centralized repository.
...
For more details on extension version refer to the [Extension Versioning page]({% link docs/1.2/extensions/versioning_of_extensions.md %}).
...
See the [installation instructions]({% link docs/1.2/extensions/installing_extensions.md %}) and the [advanced installation methods page]({% link docs/1.2/extensions/advanced_installation_methods.md %}).
