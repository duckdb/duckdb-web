...
To avoid accessing the remote endpoint for every query, you can save the data in a DuckDB table by running a [`CREATE TABLE ... AS` command]({% link docs/1.2/sql/statements/create_table.md %}#create-table--as-select-ctas). For example:
...
To query all files under a specific directory, you can use a [glob pattern]({% link docs/1.2/data/multiple_files/overview.md %}#multi-file-reads-and-globs). For example:
...
Second, set it in your DuckDB session using DuckDB’s [Secrets Manager]({% link docs/1.2/configuration/secrets_manager.md %}). DuckDB supports two providers for managing secrets:
