...
redirect_from:
- /docs/1.2/sql/functions/patternmatching
- /docs/1.2/sql/functions/patternmatching/
- /docs/1.2/sql/functions/pattern_matching
...
Additionally, the function `like_escape` has the same functionality as a `LIKE` expression with an `ESCAPE` clause, but using function syntax. See the [Text Functions Docs]({% link docs/1.2/sql/functions/char.md %}) for details.
...
The `SIMILAR TO` operator returns true or false depending on whether its pattern matches the given string. It is similar to `LIKE`, except that it interprets the pattern using a [regular expression]({% link docs/1.2/sql/functions/regular_expressions.md %}). Like `LIKE`, the `SIMILAR TO` operator succeeds only if its pattern matches the entire string; this is unlike common regular expression behavior where the pattern can match any part of the string.
...
see the [PostgreSQL compatibility page]({% link docs/1.2/sql/dialect/postgresql_compatibility.md %}).
...
DuckDB's regex support is documented on the [Regular Expressions page]({% link docs/1.2/sql/functions/regular_expressions.md %}).
