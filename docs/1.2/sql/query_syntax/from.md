...
redirect_from:
- /docs/1.2/sql/query_syntax/from
...
Semi joins provide the same logic as the [`IN` operator]({% link docs/1.2/sql/expressions/in.md %}) statement.
...
relational tables are not ordered, but imported tables such as [data frames]({% link docs/1.2/clients/python/data_ingestion.md %}#pandas-dataframes-–-object-columns)
or disk files (like [CSVs]({% link docs/1.2/data/csv/overview.md %}) or [Parquet files]({% link docs/1.2/data/parquet/overview.md %})) do have a natural ordering.
