...
layout: docu
redirect_from:
- /docs/1.2/sql/data_types/struct
...
Structs can be created using the [`struct_pack(name := expr, ...)`]({% link docs/1.2/sql/functions/struct.md %}) function, the equivalent array notation `{'name': expr, ...}`, using a row variable, or using the `row` function.
...
Retrieving a value from a struct can be accomplished using dot notation, bracket notation, or through [struct functions]({% link docs/1.2/sql/functions/struct.md %}) like `struct_extract`.
...
The same can be achieved with the star notation (`*`), which additionally allows [modifications of the returned columns]({% link docs/1.2/sql/expressions/star.md %}):
...
The `STRUCT` type can be compared using all the [comparison operators]({% link docs/1.2/sql/expressions/comparison_operators.md %}).
...
See [Struct Functions]({% link docs/1.2/sql/functions/struct.md %}).
