...
| `PARTITION_BY` | The columns to partition by using a Hive partitioning scheme, see the [partitioned writes section]({% link docs/1.2/data/partitioning/partitioned_writes.md %}). | `VARCHAR[]` | (empty) |
...
| `DATEFORMAT` | Specifies the date format to use when writing dates. See [Date Format]({% link docs/1.2/sql/functions/dateformat.md %}). | `VARCHAR` | (empty) |
...
| `TIMESTAMPFORMAT` | Specifies the date format to use when writing timestamps. See [Date Format]({% link docs/1.2/sql/functions/dateformat.md %}). | `VARCHAR` | (empty) |
...
`COPY` does not support copying between tables. To copy between tables, use an [`INSERT statement`]({% link docs/1.2/sql/statements/insert.md %}):
