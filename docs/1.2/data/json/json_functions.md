...
Note that the arrow operator `->`, which is used for JSON extracts, has a low precedence as it is also used in [lambda functions]({% link docs/1.2/sql/functions/lambda.md %}). Therefore, you need to surround the `->` operator with parentheses when expressing operations such as equality comparisons (`=`).
For example:
...
> Warning DuckDB's JSON data type uses [0-based indexing]({% link docs/1.2/data/json/overview.md %}#indexing).
...
Note that DuckDB's JSON data type uses [0-based indexing]({% link docs/1.2/data/json/overview.md %}#indexing).
