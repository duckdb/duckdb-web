...
Alternatively, create a table without specifying the schema manually using a [`CREATE TABLE .. AS SELECT` statement]({% link docs/1.2/sql/statements/create_table.md %}#create-table--as-select-ctas):
...
We can use the [`FROM`-first syntax]({% link docs/1.2/sql/query_syntax/from.md %}#from-first-syntax) to omit `SELECT *`.
...
| `auto_detect` | [Auto detect CSV parameters]({% link docs/1.2/data/csv/auto_detection.md %}). | `BOOL` | `true` |
...
| `dateformat` | [Date format]({% link docs/1.2/sql/functions/dateformat.md %}) used when parsing and writing dates. | `VARCHAR` | (empty) |
...
| `hive_partitioning` | Interpret the path as a [Hive partitioned path]({% link docs/1.2/data/partitioning/hive_partitioning.md %}). | `BOOL` | (auto-detected) |
...
| `names` or `column_names` | Column names, as a list. See [example]({% link docs/1.2/data/csv/tips.md %}#provide-names-if-the-file-does-not-contain-a-header). | `VARCHAR[]` | (empty) |
...
| `rejects_scan` | Name of the [temporary table where information on faulty scans is stored]({% link docs/1.2/data/csv/reading_faulty_csv_files.md %}#reject-scans). | `VARCHAR` | `reject_scans` |
| `rejects_table` | Name of the [temporary table where information on faulty lines is stored]({% link docs/1.2/data/csv/reading_faulty_csv_files.md %}#reject-errors). | `VARCHAR` | `reject_errors` |
...
| `sample_size` | Number of sample lines for [auto detection of parameters]({% link docs/1.2/data/csv/auto_detection.md %}). | `BIGINT` | 20480 |
...
| `timestampformat` | [Timestamp format]({% link docs/1.2/sql/functions/dateformat.md %}) used when parsing and writing timestamps. | `VARCHAR` | (empty) |
| `timestamp_format` | Alias for `timestampformat`; only available in the `COPY` statement. | `VARCHAR` | (empty) |
| `types` or `dtypes` or `column_types` | Column types, as either a list (by position) or a struct (by name). See [example]({% link docs/1.2/data/csv/tips.md %}#override-the-types-of-specific-columns). | `VARCHAR[]` or `STRUCT` | (empty) |
| `union_by_name` | Align columns from different files [by column name]({% link docs/1.2/data/multiple_files/combining_schemas.md %}#union-by-name) instead of position. Using this option increases memory consumption. | `BOOL` | `false` |
...
The `auto_type_candidates` option lets you specify the data types that should be considered by the CSV reader for [column data type detection]({% link docs/1.2/data/csv/auto_detection.md %}#type-detection).
Usage example:
...
DuckDB supports reading erroneous CSV files. For details, see the [Reading Faulty CSV Files page]({% link docs/1.2/data/csv/reading_faulty_csv_files.md %}).

## Order Preservation

The CSV reader respects the `preserve_insertion_order` [configuration option]({% link docs/1.2/configuration/overview.md %}) to [preserve insertion order]({% link docs/1.2/sql/dialect/order_preservation.md %}).
...
DuckDB can write CSV files using the [`COPY ... TO` statement]({% link docs/1.2/sql/statements/copy.md %}#copy--to).
