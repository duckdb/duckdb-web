...
Before jumping into lambdas, let's take a quick detour into DuckDB's [`LIST` type]({% link docs/current/sql/data_types/list.md %}).
...
For more details on `Vectors` and vectorized execution, please refer to the [documentation]({% link docs/current/internals/vector.md %}) and respective research papers ([1](https://15721.courses.cs.cmu.edu/spring2016/papers/p5-sompolski.pdf) and [2](https://drive.google.com/file/d/1LJeys01Ho9DREfRJhb9wHu3ssSC22Lll/view)).
...
[`list_transform`]({% link docs/current/sql/functions/lambda.md %}#list_transformlist-lambda),
[`list_filter`]({% link docs/current/sql/functions/lambda.md %}#list_filterlist-lambda),
and
[`list_reduce`]({% link docs/current/sql/functions/lambda.md %}#list_reducelist-lambda),
along with their aliases.
Each accepts a `LIST` as its first argument and a lambda function as its second argument.

Lambdas were the guest star in our [SQL Gymnastics: Bending SQL into Flexible New Shapes]({% post_url 2024-03-01-sql-gymnastics %}#creating-the-macro) blog post.
...
1. Unnest the lists while keeping the connection to their respective rows.
   We can achieve this by inventing a temporary unique identifier, such as a [`rowid`]({% link docs/current/sql/statements/select.md %}#row-ids) or a [`UUID`]({% link docs/current/sql/data_types/numeric.md %}#universally-unique-identifiers-uuids).
...
For the correlated columns `rowid` and `n`, we use [selection vectors]({% link docs/current/internals/vector.md %}), which again prevents the copying of data.
...
As established earlier, [`list_transform`]({% link docs/current/sql/functions/lambda.md %}#list_transformlist-lambda) applies a lambda function to each element of the input list and returns a new list with the transformed elements.
...
The [`list_filter` function]({% link docs/current/sql/functions/lambda.md %}#list_filterlist-lambda) filters all elements of the input list for which the lambda function returns `true`.
...
In this case, we used [`list_slice`]({% link docs/current/sql/functions/list.md %}#list_slicelist-begin-end) and [`list_concat`]({% link docs/current/sql/functions/list.md %}#list_concatlist1-list2).
...
Most recently, we've added [`list_reduce`]({% link docs/current/sql/functions/lambda.md %}#list_reducelist-lambda), which applies a lambda function to an accumulator value.
