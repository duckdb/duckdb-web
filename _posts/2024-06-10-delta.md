...
to how data is stored when DuckDB is used to write [partitioned Parquet files]({% link docs/lts/data/partitioning/partitioned_writes.md %}).
...
to a [Hive-style]({% link docs/lts/data/partitioning/hive_partitioning.md %}) partitioning scheme, the folder names are not actually used by Delta internally. Instead, the partition values are read from the metadata.
...
an [excellent Parquet reader]({% link docs/lts/data/parquet/overview.md %}). Also, DuckDB already has support for a [variety]({% link docs/lts/core_extensions/httpfs/hugging_face.md %}) [of]({% link docs/lts/core_extensions/httpfs/s3api.md %}) [filesystems]({% link docs/lts/core_extensions/azure.md %}) with its own [credential management system]({% link docs/lts/configuration/secrets_manager.md %}). By using a library like
...
| Delta extension   | DuckDB's loadable [Delta extension]({% link docs/lts/core_extensions/delta.md %})   |
| Parquet extension | DuckDB's loadable [Parquet extension]({% link docs/lts/data/parquet/overview.md %}) |
...
| API                    | Description                                                                                                                                                                        |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FileSystem`           | DuckDB's API for I/O (for local files, [Azure]({% link docs/lts/core_extensions/azure.md %}), [S3]({% link docs/lts/core_extensions/httpfs/s3api.md %}), etc.)               |
| `TableFunction`        | DuckDB's API for table functions (e.g., [`read_parquet`]({% link docs/lts/data/parquet/overview.md %}), [`read_csv`]({% link docs/lts/guides/file_formats/csv_import.md %})) |
...
3. **All I/O will go through DuckDB's `FileSystem` API.** This means that all file systems ([Azure]({% link docs/lts/core_extensions/azure.md %}), [S3]({% link docs/lts/core_extensions/httpfs/s3api.md %}), etc.) that are available to DuckDB, are available to scan with.
...
Using the Delta extension in DuckDB is very simple, as it is distributed as one of the core DuckDB extensions, and available for [autoloading]({% link docs/lts/core_extensions/overview.md %}#autoloading-extensions).
...
For details and info on newly added features, keep an eye on the Delta extension [docs]({% link docs/lts/core_extensions/delta.md %}) and [repository](https://github.com/duckdb/duckdb-delta).
...
We hope you give the [Delta extension]({% link docs/lts/core_extensions/delta.md %}) a try and look forward to any feedback from the community! Also, if you're attending the
