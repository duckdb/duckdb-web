...
to a [Hive-style](/docs/data/partitioning/hive_partitioning) partitioning scheme, the folder names are not actually used by Delta internally. Instead, the partition values are read from the metadata.
...
In this Diagram, we can see all 4 components involved in the processing of query containing a `delta_scan` table function. The arrows represent the communication that occurs
between the components across the 4 APIs. Now when reading a Delta Table, we can see that the Metadata is handled on the right side going through the Delta Kernel. On the left side
...
3. **All I/O will go through DuckDB's `FileSystem` API.** This means that all file systems ([Azure](/docs/extensions/azure), [S3](/docs/extensions/httpfs/s3api), etc.) that are available to DuckDB, are available to scan with.
