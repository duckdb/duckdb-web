...
<div align="center">
<img src="{% link images/blog/delta/lakehouse_arch.png %}"
alt="Image describing the lakehouse architecture"
width="300"
/></div>
...
to how data is stored when DuckDB is used to write [partitioned Parquet files]({% link docs/current/data/partitioning/partitioned_writes.md %}).
...
to a [Hive-style]({% link docs/current/data/partitioning/hive_partitioning.md %}) partitioning scheme, the folder names are not actually used by Delta internally. Instead, the partition values are read from the metadata.
...
an [excellent Parquet reader]({% link docs/current/data/parquet/overview.md %}). Also, DuckDB already has support for a [variety]({% link docs/current/core_extensions/httpfs/hugging_face.md %}) [of]({% link docs/current/core_extensions/httpfs/s3api.md %}) [filesystems]({% link docs/current/core_extensions/azure.md %}) with its own [credential management system]({% link docs/current/configuration/secrets_manager.md %}). By using a library like
...
<div align="center">
<img src="{% link images/blog/delta/kernel.png %}"
...
| Delta extension   | DuckDB's loadable [Delta extension]({% link docs/current/core_extensions/delta.md %})   |
| Parquet extension | DuckDB's loadable [Parquet extension]({% link docs/current/data/parquet/overview.md %}) |
...
| API                    | Description                                                                                                                                                                        |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FileSystem`           | DuckDB's API for I/O (for local files, [Azure]({% link docs/current/core_extensions/azure.md %}), [S3]({% link docs/current/core_extensions/httpfs/s3api.md %}), etc.)               |
| `TableFunction`        | DuckDB's API for table functions (e.g., [`read_parquet`]({% link docs/current/data/parquet/overview.md %}), [`read_csv`]({% link docs/current/guides/file_formats/csv_import.md %})) |
...
<div align="center">
    <img src="{% link images/blog/delta/delta_ext_overview-light.svg %}"
         alt="Diagram showing operation of delta_scan"
         width="700"
         class="lightmode-img"
         />
    <img src="{% link images/blog/delta/delta_ext_overview-dark.svg %}"
         alt="Diagram showing operation of delta_scan"
         width="700"
         class="darkmode-img"
         />
...
3. **All I/O will go through DuckDB's `FileSystem` API.** This means that all file systems ([Azure]({% link docs/current/core_extensions/azure.md %}), [S3]({% link docs/current/core_extensions/httpfs/s3api.md %}), etc.) that are available to DuckDB, are available to scan with.
...
Using the Delta extension in DuckDB is very simple, as it is distributed as one of the core DuckDB extensions, and available for [autoloading]({% link docs/current/core_extensions/overview.md %}#autoloading-extensions).
...
For details and info on newly added features, keep an eye on the Delta extension [docs]({% link docs/current/core_extensions/delta.md %}) and [repository](https://github.com/duckdb/duckdb-delta).
...
We hope you give the [Delta extension]({% link docs/current/core_extensions/delta.md %}) a try and look forward to any feedback from the community! Also, if you're attending the
