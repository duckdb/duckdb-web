...
This is because the more approximate time buckets allow the Hilbert encoding to separate origins into different rowgroups more effectively.
...
This is because the origin and destination filters are much more effective at skipping rowgroups when the timestamp ordering is very approximate.
...
CREATE OR REPLACE FUNCTION rowgroup_counts(table_name, column_list) AS TABLE (
    ...
    WITH by_rowgroup_id AS (
        FROM ' || dq(table_name) || '
        SELECT
        ceiling((count(*) over ()) / 122880) AS total_rowgroups,
    ...
    SELECT
        ...
        nq_concat(list_transform(column_list, (i) -> ' when grouping('||dq(i)||') = 0 then alias('||dq(i)||') '),' ')
            || ' end AS column_name,
        coalesce(*columns(* exclude (rowgroup_id, total_rowgroups))) AS column_value,
        first(total_rowgroups) AS total_rowgroups,
