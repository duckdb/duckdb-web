...
We implemented the application using the [DuckDB Swift client]({% link docs/lts/clients/swift.md %}) and loaded the benchmark on the phone, all 30 GB of it.
...
We ran DuckDB in the [Termux terminal emulator](https://termux.dev/en/). We compiled DuckDB [CLI client]({% link docs/lts/clients/cli/overview.md %}) from source following the [Android build instructions]({% link docs/lts/dev/building/android.md %}) and ran the experiments from the command line.
