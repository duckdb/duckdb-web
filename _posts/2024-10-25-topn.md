...
The machine used for benchmarking was an M1 Macbook Pro with 16GB RAM.

| Scale Factor | Metric | Qualify | max_by | Performance Improvement | 
|-----------:|----------:|----------:|----------:|----------:|
| 1 | Total Time | 0.58s | 0.24s | 2.4x |
| 5 | Total Time | 6.15s | 1.26s | 4.9x |
| 10 | Total Time | 36.8s | 25.4s | 1.4x |
| 1 | Memory Usage | 0.2GB | 1.7GB | 8.5x |
| 5 | Memory Usage | 7.9GB | 1.5GB | 5.3x |
| 10 | Memory Usage | 15.7GB | 17.1GB | 0.9x |

We can see that in each of these situations, the `max_by` approach is faster, in some cases nearly 5x faster!
...
At a high level, in cases with more rows within a group, favor `max_by`, as the sorting will be more compute intensive in those situations.
If memory is constrained, `max_by` may also offer benefits.
However, if there are approximately as many groups as there are rows, consider `QUALIFY` since there will be few rows to sort per group.
