...
For example, using the scale factor 1 (SF1) data set of the [TPC-H benchmark]({% link docs/current/core_extensions/tpch.md %}):
...
> The queries in this post make extensive use of DuckDB's [`FROM`-first syntax]({% link docs/current/sql/query_syntax/from.md %}#from-first-syntax).
...
In most databases, the way to filter to the top N within a group is to use a [window function]({% link docs/current/sql/functions/window_functions.md %}) and a [common table expression (CTE)]({% link docs/current/sql/query_syntax/with.md %}).
...
In DuckDB, this can be simplified using the [`QUALIFY` clause]({% link docs/current/sql/query_syntax/qualify.md %}).
...
- The [`max_by` function]({% link docs/current/sql/functions/aggregates.md %}#max_byarg-val-n) (also known as `arg_max`)
- The [`unnest` function]({% link docs/current/sql/query_syntax/unnest.md %})
- Automatically packing an entire row into a [`STRUCT` column]({% link docs/current/sql/data_types/struct.md %}#creating-structs)
