...
Among other things, DuckDB's design is different from conventional data management systems because DuckDB's query processing engine can run on nearly arbitrary data sources without needing to copy the data into its own storage format. For example, DuckDB can currently directly run queries on [Parquet files](/docs/data/parquet), [CSV files](/docs/data/csv/overview), [SQLite files](https://github.com/duckdblabs/sqlite_scanner), [Pandas](/docs/guides/python/sql_on_pandas), [R](/docs/api/r#efficient-transfer) and [Julia](/docs/api/julia#scanning-dataframes) data frames as well as [Apache Arrow sources](/docs/guides/python/sql_on_arrow). This new extension adds the capability to directly query PostgreSQL tables from DuckDB. 
