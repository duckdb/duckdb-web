...
You often need to read files in various formats (such as CSV, JSONL, and Parquet) when working with data. As of version v0.10.3, DuckDB has native support for `hf://` paths as part of the [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}), allowing easy access to all these formats.
...
To avoid accessing the remote endpoint for every query, you can save the data in a DuckDB table by running a [`CREATE TABLE ... AS` command]({% link docs/current/sql/statements/create_table.md %}#create-table--as-select-ctas). For example:
...
To query all files under a specific format, you can use a [glob pattern]({% link docs/current/data/multiple_files/overview.md %}#multi-file-reads-and-globs). Here’s how you can count the rows in all files that match the pattern `*.parquet`:
...
Second, set it in your DuckDB session using DuckDB’s [Secrets Manager]({% link docs/current/configuration/secrets_manager.md %}). DuckDB supports two providers for managing secrets:
