...
Users will want to move their data to another device, back it up, or open it with a different tool. DuckDB-Wasm itself [cannot move files into or out of OPFS]({% link docs/current/clients/wasm/instantiation.md %}#persistence-with-opfs) yet, but the database is a plain DuckDB file and the browser's OPFS API lets you read it back as bytes:
