...
<img src="{% link images/blog/jason-duck.jpg %}" alt="JSON is not scary anymore! Jason IS scary though, even as a duck." width="180"/>

DuckDB has a [JSON extension]({% link docs/current/data/json/overview.md %}) that can be installed and loaded through SQL:
...
To demonstrate these, we will read [`todos.json`]({% link data/json/todos.json %}), a [fake TODO list](https://jsonplaceholder.typicode.com/todos) containing 200 fake TODO items (only the first two items are shown):
...
This function is similar to our `read_csv` function, which [automatically infers column names and types for CSV files]({% link docs/current/data/csv/auto_detection.md %}).
...
You can also directly query the JSON files from GH Archive using DuckDB's [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}), but we will be querying the files multiple times, so it is better to download them in this case.
...
As expected, Mark Raasveldt (Mytherin, co-founder of DuckLabs) was the most active!
My activity (lnkuiper, software engineer at DuckLabs) also shows up.
