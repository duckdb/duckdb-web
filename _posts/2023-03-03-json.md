...
DuckDB has a [JSON extension]({% link docs/lts/data/json/overview.md %}) that can be installed and loaded through SQL:
...
This function is similar to our `read_csv` function, which [automatically infers column names and types for CSV files]({% link docs/lts/data/csv/auto_detection.md %}).
...
You can also directly query the JSON files from GH Archive using DuckDB's [`httpfs` extension]({% link docs/lts/core_extensions/httpfs/overview.md %}), but we will be querying the files multiple times, so it is better to download them in this case.
