...
DuckDB provides support for two distinct types of Python UDFs, differing in the Python object used for communication between [DuckDB's native data types]({% link docs/current/sql/data_types/overview.md %}) and the Python process. These communication layers include support for [Python built-in types]({% link docs/current/sql/data_types/overview.md %}) and [PyArrow Tables](https://arrow.apache.org/docs/python/generated/pyarrow.Table.html).
