...
![boxplot-labels](/images/blog/jupysql/boxplot-labels.png)
...
```sql
-- We calculate the percentiles all at once and then convert from list format into separate columns
-- (This improves performance by reducing duplicate work)
WITH stats as (
  SELECT
...
```sql magic_args="--save no-outliers --no-execute"
SELECT *
FROM 'yellow_tripdata_2022-01.parquet'
