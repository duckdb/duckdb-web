...
In addition to the traditional [`UNION`]({% link docs/current/sql/query_syntax/setops.md %}#union) and [`UNION ALL`]({% link docs/current/sql/query_syntax/setops.md %}#union-all-bag-semantics) operators, DuckDB adds both [`UNION BY NAME` and `UNION ALL BY NAME`]({% link docs/current/sql/query_syntax/setops.md %}#union-all-by-name).
...
And finally, `UNION ALL` [preserves the original row order]({% link docs/current/sql/dialect/order_preservation.md %}).
...
* [`read_csv`]({% link docs/current/data/csv/overview.md %}#parameters)
* [`read_json`]({% link docs/current/data/json/loading_json.md %}#parameters)
* [`read_parquet`]({% link docs/current/data/parquet/overview.md %}#parameters)
...
> DuckDB's extensions to read lakehouse table formats like [Delta]({% link docs/current/core_extensions/delta.md %}) and [Iceberg]({% link docs/current/core_extensions/iceberg/overview.md %}) handle schema evolution within the formats' own metadata, so `union_by_name` is not needed.

## Inserting Data by Name

Another use case for vertically stacking data is when inserting into an existing table.
The DuckDB syntax of [`INSERT INTO ⟨my_table⟩ BY NAME`{:.language-sql .highlight}]({% link docs/current/sql/statements/insert.md %}#insert-into--by-name) offers the same flexibility of referring to columns by name rather than by position.
