...
| groupby #3  |   `SELECT id3, sum(v1) AS v1, mean(v3) AS v3 FROM tbl GROUP BY id3`                 |      Sum and mean aggregation function over many small cardinality groups, grouped by varchar                   |
...
| groupby #6  |   `SELECT id4, id5, quantile_cont(v3, 0.5) AS median_v3, stddev(v3) AS sd_v3 FROM tbl GROUP BY id4, id5`  | over medium cardinality group, grouped by integers                                        |
| groupby #7  |   `SELECT id3, max(v1)-min(v2) AS range_v1_v2 FROM tbl GROUP BY id3`                |      Range selection over small vardinality groups, grouped by integer                                          |
...
| groupby #9  |   `SELECT id2, id4, pow(corr(v1, v2), 2) AS r2 FROM tbl GROUP BY id2, id4`          |      Arithmetic over medium sized groups, grouped by varchar, integer.                                                         |
| groupby #10 |   `SELECT id1, id2, id3, id4, id5, id6, sum(v3) AS v3, count(*) AS count FROM tbl GROUP BY id1, id2, id3, id4, id5, id6`  | Many many small groups, the number of groups is the cardinality of the dataset           |
...
<div style="position:relative; padding-bottom:10%; width: 100%">
	<iframe src="https://tmonster.github.io/h2oai-db-benchmark/"  title="h2o db benchmmark"></iframe>
</div>
