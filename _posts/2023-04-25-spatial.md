...
That said, we're aiming to implement more of the core functions and algorithms natively in the future to enable tighter integration with DuckDBs execution and memory management.

We also provide a set of non-standard specialized columnar DuckDB native geometry types such as `POINT_2D`, `BOX_2D`, etc. that should provide better compression and faster execution in exchange for some flexibility, but work around these are still very much experimental.
...
Let's start by loading the spatial extension and the [parquet extension](https://duckdb.org/docs/data/parquet/overview.html#installing-and-loading-parquet-extension) so we can import the NYC taxi ride data provided in parquet format, as well as the accompanying taxi zone data from a shapefile, using the `ST_Read` table function provided by the spatial extension. We then create a table for the rides and a table for the zones. Note that `ST_Read` produces a table with a `wkb_geometry` column that contains the geometry data encoded as a WKB (Well-Known Binary) blob, which we then convert to the `GEOMETRY` type using the `ST_GeomFromWKB` function. 
...
Trips with a distance shorter than the aerial distance are likely to be erroneous, so we use this query to filter out some bad data. 
...
And there we have it! The full set of currently supported functions and their implementation status can be found in [this table](https://github.com/duckdblabs/duckdb_spatial). If you have a particular function or operation in mind thats not yet implemented but you would like to see supported, please let us know in the [GitHub discussions](https://github.com/duckdblabs/duckdb_spatial/discussions/categories/ideas)
...
Please take a look at the [GitHub repository](https://github.com/duckdblabs/duckdb_spatial) for the full roadmap and to see what we're currently working on.
