...
author: "Jeff Raymakers, Gábor Szárnyas"
...
DuckDB can be used from [many popular languages]({% link docs/current/clients/overview.md %}) and runs on a [wide variety of platforms]({% link why_duckdb.md %}#portable).
The included [Command Line Interface (CLI)]({% link docs/current/clients/cli/overview.md %}) provides a convenient way to interactively run SQL queries from a terminal window,
and several [third-party tools](https://github.com/davidgasquez/awesome-duckdb#readme) offer more sophisticated UIs.
...
> The DuckDB UI is the result of a [collaboration]({% link faq.md %}#how-are-duckdb-the-duckdb-foundation-duckdb-labs-and-motherduck-related) between [DuckDB Labs](https://ducklabs.com) and [MotherDuck](https://motherduck.com/) and is shipped as part of the `ui` extension.

<video muted controls width="700">
...
You can also run the following SQL command from a [DuckDB client]({% link docs/current/clients/overview.md %}) (e.g., CLI, Python, Java, etc.):
...
Support for the UI is implemented in a [DuckDB extension]({% link docs/current/core_extensions/overview.md %}).
...
See the [UI extension documentation]({% link docs/current/core_extensions/ui.md %}) for more details.
