...
excerpt: "The DuckDB team and MotherDuck are excited to announce the release of a built-in local UI for DuckDB."
tags: ["using DuckDB"]
---

The DuckDB project was built to make it [simple](https://duckdb.org/why_duckdb#simple) to leverage [modern database technology](https://duckdb.org/why_duckdb#feature-rich).
DuckDB can be used from [many popular languages](https://duckdb.org/docs/stable/clients/overview) and runs on a [wide variety of platforms](https://duckdb.org/why_duckdb#portable).
The included [Command Line Interface (CLI)](https://duckdb.org/docs/stable/clients/cli/overview)provides a convenient way to interactively run SQL queries from a terminal window,
...
However, it can be cumbersume for working with lengthy SQL queries, and its data exploration tools are limited.
...
> The DuckDB UI is the result of a [collaboration](https://duckdb.org/faq#how-are-duckdb-the-duckdb-foundation-duckdb-labs-and-motherduck-related) between [DuckDB Labs](https://duckdblabs.com) and [MotherDuck](https://motherduck.com/).
...
You can also run the following SQL command from a [DuckDB client](https://duckdb.org/docs/clients/overview) (e.g., CLI, Python, Java, etc.):
...
Support for the UI is implemented in a [DuckDB extension](https://duckdb.org/docs/stable/extensions/overview).
...
See the [UI extension documentation](https://duckdb.org/docs/stable/extensions/ui) for more details.
...
The DuckDB UI is also fully open source: visit the [`duckdb/duckdb-ui` repository](https://github.com/duckdb/duckdb-ui) if you want to dive in deeper.
