...
DuckDB can be used from [many popular languages]({% link docs/lts/clients/overview.md %}) and runs on a [wide variety of platforms]({% link why_duckdb.md %}#portable).
The included [Command Line Interface (CLI)]({% link docs/lts/clients/cli/overview.md %}) provides a convenient way to interactively run SQL queries from a terminal window,
...
You can also run the following SQL command from a [DuckDB client]({% link docs/lts/clients/overview.md %}) (e.g., CLI, Python, Java, etc.):
...
Support for the UI is implemented in a [DuckDB extension]({% link docs/lts/core_extensions/overview.md %}).
...
See the [UI extension documentation]({% link docs/lts/core_extensions/ui.md %}) for more details.
