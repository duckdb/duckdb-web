...
-- Print the new plan using EXPLAIN to verify that we are using
-- a nested loop join
...
Since checking if two bounding boxes intersect can be expressed as a set of simple less-than and greater-than comparisons, we can use DuckDB's existing [inequality join]({% post_url 2022-05-27-iejoin %}#inequality-join-iejoin) functionality to perform this join efficiently! To implement this, we made the following changes to `spatial`:
...
|                10,000,000 |          310.3 s |               21.2 s |       14.5 s |
|                58,033,724 |         1799.6 s |              107.6 s |       28.7 s |

As you can see, the `SPATIAL_JOIN` operator is able to execute the full 58 million row dataset faster than the original naive nested loop join could execute the 1 million row dataset. That's a 58× improvement!
But, more importantly, it also _scales_ much better. Compared to the `PIECEWISE_MERGE_JOIN`, the execution time only doubles as the input size increases to almost 6×, instead of being multiplied by a similar factor.
