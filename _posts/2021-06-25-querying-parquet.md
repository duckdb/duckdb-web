...
First we look at some rows in the dataset. There are three Parquet files in the `taxi/` folder. [DuckDB supports the globbing syntax]({% link docs/lts/data/parquet/overview.md %}), which allows it to query all three files simultaneously.
...
Note that [DuckDB also has support for writing Parquet files]({% link docs/lts/data/parquet/overview.md %}#writing-to-parquet-files) using the COPY statement.
