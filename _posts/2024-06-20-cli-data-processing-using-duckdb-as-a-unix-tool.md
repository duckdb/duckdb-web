...
In the process, we will show off some cool features such as DuckDB's [powerful CSV reader]({% link docs/current/data/csv/overview.md %}) and the [positional join operator](#duckdb-positional-join).
Let's get started!
...
Second, it has a standalone [command line client]({% link docs/current/clients/cli/overview.md %}), which can consume and produce CSV files _(work together),_
...
Another attractive feature of DuckDB is that it offers an interactive shell, which aids quick debugging. Moreover, DuckDB is available in [several host languages]({% link docs/current/clients/overview.md %}) as well as in the browser [via WebAssembly](https://shell.duckdb.org/), so if you ever decide to use your SQL scripts outside of the shell, DuckDB SQL scripts can be ported to a wide variety of environments without any changes.
...
We use DuckDB v1.0.0 and run it in [in-memory mode]({% link docs/current/connect/overview.md %}#in-memory-database).
...
DuckDB detects that this is a CSV file and invokes the [`read_csv` function]({% link docs/current/data/csv/overview.md %}#csv-functions), which automatically infers the CSV file's dialect (delimiter, presence of quotes, etc.) as well as the schema of the table.
...
To make the output of the solutions using Unix tools and DuckDB equivalent, we wrap the query into a [`COPY ... TO` statement]({% link docs/current/sql/statements/copy.md %}#copy--to):
...
The DuckDB solution reads the CSV files, projects the `city` fields and applies the [`INTERSECT ALL` clause]({% link docs/current/sql/query_syntax/setops.md %}#intersect-all-bag-semantics) to calculate the intersection:
...
In DuckDB, we can use a [`POSITIONAL JOIN`]({% link docs/current/sql/query_syntax/from.md %}#positional-joins).
This join type is one of DuckDB's [SQL extensions]({% link docs/current/sql/dialect/friendly_sql.md %}) and it provides a concise syntax to combine tables row-by-row based on each row's position in the table.
Joining the two tables together using `POSITIONAL JOIN` results in two `city` columns – we use the [`EXCLUDE` clause]({% link docs/current/sql/expressions/star.md %}#exclude-clause) to remove the duplicate column:
...
To answer the first question, we can use [`LIKE` for pattern matching]({% link docs/current/sql/functions/pattern_matching.md %}).
...
For the second question, we use [string slicing]({% link docs/current/sql/functions/text.md %}#stringbeginend) to extract the first three characters, [`upper`]({% link docs/current/sql/functions/text.md %}#upperstring) to ensure case-insensitivity, and `NOT LIKE` for the negative condition:
...
In both of these queries, we used the [`FROM`-first syntax]({% link docs/current/sql/query_syntax/from.md %}#from-first-syntax).
...
In DuckDB, we load the CSV files and connect them using the [`NATURAL JOIN` clause]({% link docs/current/sql/query_syntax/from.md %}#natural-joins), which joins on column(s) with the same name.
To ensure that the result matches with that of the Unix solution, we use the [`ORDER BY ALL` clause]({% link docs/current/sql/query_syntax/orderby.md %}#order-by-all), which sorts the result on all columns, starting from the first one, and stepping through them for tie-breaking to the last column.
...
The presence of the `https://` prefix triggers DuckDB to load the [`httpfs` extension]({% link docs/current/core_extensions/httpfs/overview.md %}) and use it to fetch the JSON document.
We use the [`replace` function]({% link docs/current/sql/functions/text.md %}#replacestring-source-target) to substitute the spaces with underscores,
and the [`regexp_replace` function]({% link docs/current/sql/functions/text.md %}#regexp_replacestring-pattern-replacement) for the replacement using a regular expression.
(We could have also used string formatting functions such as [`format`]({% link docs/current/sql/functions/text.md %}#fmt-syntax) and [`printf`]({% link docs/current/sql/functions/text.md %}#printf-syntax)).
...
In DuckDB, we use the [`read_json` function]({% link docs/current/data/json/overview.md %}), invoking it with the remote HTTPS endpoint's URL.
...
In DuckDB, the built-in CSV reader also supports [compressed CSV files]({% link docs/current/data/csv/overview.md %}#parameters), so we can use that without any extra configuration.
