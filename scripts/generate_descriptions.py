...
benchmark_runner = os.path.join(
    duckdb_base, 'build', 'release', 'benchmark', 'benchmark_runner'
)
...
def format_sql(sql):
    with open('format.tmp.sql', 'w+') as f:
        f.write(sql)
    return get_output(
        ['pg_format', '--keyword-case', '2', '--spaces', '4', 'format.tmp.sql']
    )
...
def generate_benchmark_info(benchmark_name, con, c):
    ...
    c.execute(
        'INSERT INTO descriptions (benchmark, description) VALUES (?,?)',
        (display_name, result_html),
    )
...
for benchmark_name in [
    x.strip() for x in benchmark_list.split('\n') if len(x.strip()) > 0
]:
