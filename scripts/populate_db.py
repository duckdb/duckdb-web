...
def write_benchmark_info(benchmark, description):
    ...
    # write to db
    c.execute(
        "INSERT INTO benchmarks (name, groupname, description) VALUES (?, ?, ?)",
        (benchmark, groupname, description),
    )
    # now fetch the id
...
for benchmark_dir in commit_list:
    ...
    proc = subprocess.Popen(
        ['git', 'show', '-s', '--format=%ci', commit_hash], stdout=subprocess.PIPE
    )
    date = proc.stdout.read().decode('utf8').strip()
    proc = subprocess.Popen(
        ['git', 'show', '-s', '--format=%B', commit_hash], stdout=subprocess.PIPE
    )
    ...
    for bname in benchmarks.keys():
        ...
        if len(error_msg) > 0:
            ...
            c.execute(
                "INSERT INTO timings (benchmark_id, hash, success, median, timings, profile, stdout, stderr) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    benchmark_id,
                    commit_hash,
                    True,
