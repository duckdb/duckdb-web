...
def write_commit(f, commit):
    f.write(
        '<a href="https://github.com/duckdb/duckdb/commit/%s">%s</a>'
        % (commit, commit[:4])
    )
...
def pack_commit(c, commit_hash, written_commits):
    ...
    graph_path = os.path.join(
        '_includes', 'benchmark_logs', '%s-graph.json' % (commit_hash,)
    )
    stderr_path = os.path.join(
        '_includes', 'benchmark_logs', '%s-stderr.json' % (commit_hash,)
    )
    stdout_path = os.path.join(
        '_includes', 'benchmark_logs', '%s-stdout.json' % (commit_hash,)
    )
    ...
    if os.path.isfile(graph_path):
        ...
        result_dict = {}
        c.execute(
            "SELECT name, profile, stdout, stderr FROM timings JOIN benchmarks ON benchmark_id=id WHERE hash=?",
            (commit_hash,),
        )
        for entry in c.fetchall():
            name = entry[0]
    ...
    result_dict = {}
    graph_data = {'meta_info': {}, 'graph_json': {}}
    stdout = {}
    stderr = {}
    c.execute(
        "SELECT name, profile, stdout, stderr FROM timings JOIN benchmarks ON benchmark_id=id WHERE hash=?",
...
def create_html(database):
    ...
    for benchmark_info in c.fetchall():
        ...
        with open(
            os.path.join('_includes', 'individual_results', benchmark_name + '.html'),
            'w+',
        ) as f:
            ...
            c.execute(
                "SELECT commits.hash, date, median, success, error, commits.message FROM timings JOIN commits ON commits.hash=timings.hash WHERE benchmark_id=? ORDER BY date DESC;",
                (benchmark_id,),
            )
            results = c.fetchall()
        ...
        with open(
            os.path.join('benchmarks', 'individual_results', benchmark_name + '.md'),
            'w+',
        ) as f:
...
---"""
    ...
    for groupname in groups:
        ...
        c.execute(
            "SELECT commits.hash FROM commits JOIN timings ON commits.hash=timings.hash WHERE benchmark_id=? ORDER BY commits.date DESC LIMIT 15",
            (benchmark_id,),
        )
        ...
        c.execute(
            "SELECT name, id FROM benchmarks WHERE groupname=? ORDER BY 1;",
            (groupname,),
        )
        benchmark_results = {}
        ...
        for commit_hash in hashes:
            ...
            c.execute(
                "SELECT benchmark_id, success, error, median FROM timings WHERE hash=? AND benchmark_id IN (%s)"
                % (benchmark_in_list,),
                (commit_hash,),
            )
            for entry in c.fetchall():
                ...
                benchmark_results[benchmark_name][commit_hash] = (
                    result_html,
                    table_class,
                )
        ...
        out_html = os.path.join(
            '_includes', 'benchmark_results', normalized_group + '.html'
        )
...
"""
        ...
        with open(out_html, 'w+') as f:
            ...
            for entry in benchmark_list:
                ...
                f.write(
                    '<a href="/benchmarks/individual_results/%s">%s</a>'
                ...
                for commit_hash in hashes:
                    if commit_hash in benchmark_results[benchmark_name]:
                        (html, table_class) = benchmark_results[benchmark_name][
                            commit_hash
                        ]
                    else:
                        html = ""
