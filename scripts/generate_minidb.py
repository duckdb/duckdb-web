...
for benchmark_info in benchmarks:
    ...
    if len(c.fetchall()) > 0:
        continue
    c.execute(
        'INSERT INTO benchmarks (id, name, groupname, description) VALUES (?, ?, ?, ?)',
        benchmark_info,
    )
...
for timing_info in timings:
    c.execute(
        'SELECT * FROM timings WHERE hash=? AND benchmark_id=?',
        (timing_info[0], timing_info[1]),
    )
    if len(c.fetchall()) > 0:
        continue
    c.execute(
        'INSERT INTO timings (hash, benchmark_id, success, median, timings, error) VALUES (?, ?, ?, ?, ?, ?)',
        timing_info,
    )
