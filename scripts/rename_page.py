...
parser = argparse.ArgumentParser(description='Rename a page in the docs, propagate rename to archived versions and add correct redirects.')
parser.add_argument(
    '--source',
    dest='source_file',
    action='store',
...
parser.add_argument('--execute', dest='execute', action='store_true', help='The unittest filter to apply', default=False)
...
if not source_page.startswith('docs/') or not target_page.startswith('docs/'):
	raise Exception(f"Both source ({source_page}) and target ({target_page}) need to be in docs/ subdirectory")
