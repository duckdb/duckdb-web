...
def flush_group(c, current_group, subgroup, display_name, description):
    c.execute(
        "select count(*) from groups where name = ? and subgroup=?",
    ...
    if count == 0:
        # no entry yet
        c.execute(
            'insert into groups (name, display_name, subgroup, description) values (?,?,?,?)',
            (current_group, display_name, subgroup, description),
        )
    else:
        # entry exists, update it
        c.execute(
            'update groups set display_name=?, subgroup=?, description=? where name=?',
            (display_name, subgroup, description, current_group),
        )
...
for line in lines:
    if len(line) == 0:
        flush_group(
            c, current_group, current_subgroup, display_name, ' '.join(description)
        )
        current_group = None
        current_subgroup = None
        display_name = None
