...
excerpt: |
  ...
  Authorize untrusted read-only SQL from tenants, LLM agents, and dashboards with table and function policies, structured validation results, and optional connection enforcement.

extension:
  name: gatekeeper
  description: Authorize untrusted read-only SQL from tenants, LLM agents, and dashboards with table and function policies, structured validation results, and optional connection enforcement.
  version: 0.2.0
  language: C++
  build: cmake
  license: MIT
...
repo:
  ...
  ref: 6b1e80b72655cd201ddb80183c31d7179f0606d6
...
docs:
  ...
  extended_description: |
    Gatekeeper checks whether untrusted SQL stays within your table and function policy.
    It is built for multi-tenant analytics, LLM agents, and embedded dashboards where you
    control the database but not the queries.

    - **Validate a statement** without executing it and get structured diagnostics.
    - **Set a global policy** that per-request options can narrow, never widen.
    - **Optionally enforce on a connection**, with an audit log and log-only rollout mode.
    ...
    See the [trusted-definition rules](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/docs/security.md#function-enforcement-and-trusted-expansion)
    ...
    See the [enforcement guide](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/docs/security.md#enforced-connections)
    ...
    the host will turn it off. See the [audit log](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/docs/security.md#audit-log)
    and [log-only guide](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/docs/security.md#log-only-mode).
    ...
    [function policy](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/docs/security.md#never-bind-functions)
    ...
    Read the [security model](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/docs/security.md)
    for the full boundaries and setup requirements. Report suspected bypasses through
    [SECURITY.md](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/SECURITY.md).
    ...
    See [build-pin maintenance](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/inventories/README.md#repinning-the-engine)
    and [Wasm setup](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/test/wasm/README.md).
    ...
    0.2.0. The plain row is the client round trip plus the engine's own work; in parentheses,
    ...
    [scripts/benchmark.py](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/scripts/benchmark.py).

    Full documentation, including a Python integration example, is in the
    [README](https://github.com/nozzle/duckdb-gatekeeper/blob/v0.2.0/README.md).
